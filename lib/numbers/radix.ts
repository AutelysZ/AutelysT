...
export function convertRadix(value: string, fromBase: number, toBase: number, options: RadixOptions = {}): string {
  ...
  while (temp > 0) {
    ...
    const digit = remainder < 10 ? String(remainder) : String.fromCharCode(55 + remainder)
    result = digit + result
...
export function toBase60(value: bigint, padding: 0 | 2 = 0): string {
  ...
  while (temp > 0) {
    ...
    parts.unshift(padding === 2 ? part.toString().padStart(2, "0") : part.toString())
    temp = temp / BigInt(60)
  }
