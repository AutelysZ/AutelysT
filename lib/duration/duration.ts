...
export function parseDuration(input: string): {
  ...
  if (!remaining) {
    return {
      components: { ...EMPTY_DURATION },
      error: "Duration cannot be just 'P'",
    };
  }
  ...
  while ((dateMatch = dateRegex.exec(datePart)) !== null) {
    ...
    if (usedDateDesignators.has(designator)) {
      return {
        components: { ...EMPTY_DURATION },
        error: `Duplicate designator '${designator}'`,
      };
    }
  ...
  const cleanedDatePart = datePart
    .replace(/(\d+(?:\.\d+)?)\s*[YMWDymwd]/g, "")
    .trim();
  if (cleanedDatePart && !/^\s*$/.test(cleanedDatePart)) {
    return {
      components: { ...EMPTY_DURATION },
      error: `Invalid date component: '${cleanedDatePart}'`,
    };
  }
  ...
  if (timePart) {
    ...
    const cleanedTimePart = timePart
      .replace(/(\d+(?:\.\d+)?)\s*[HMShms]/g, "")
      .trim();
    if (cleanedTimePart && !/^\s*$/.test(cleanedTimePart)) {
      return {
        components: { ...EMPTY_DURATION },
        error: `Invalid time component: '${cleanedTimePart}'`,
      };
    }
    ...
    if (!usedTimeDesignators.size) {
      return {
        components: { ...EMPTY_DURATION },
        error: "Time designator 'T' present but no time components",
      };
    }
  }
  ...
  if (!hasAny && remaining !== "0D" && remaining !== "T0S") {
    ...
    if (!/^0[YMWD]$/i.test(remaining) && !/^T0[HMS]$/i.test(remaining)) {
      return {
        components: { ...EMPTY_DURATION },
        error: "No valid duration components found",
      };
    }
  }
...
export function toHumanReadable(components: DurationComponents): string {
  ...
  if (components.years) {
    parts.push(
      `${formatNumber(components.years)} ${components.years === 1 ? "year" : "years"}`,
    );
  }
  if (components.months) {
    parts.push(
      `${formatNumber(components.months)} ${components.months === 1 ? "month" : "months"}`,
    );
  }
  if (components.weeks) {
    parts.push(
      `${formatNumber(components.weeks)} ${components.weeks === 1 ? "week" : "weeks"}`,
    );
  }
  if (components.days) {
    parts.push(
      `${formatNumber(components.days)} ${components.days === 1 ? "day" : "days"}`,
    );
  }
  if (components.hours) {
    parts.push(
      `${formatNumber(components.hours)} ${components.hours === 1 ? "hour" : "hours"}`,
    );
  }
  if (components.minutes) {
    parts.push(
      `${formatNumber(components.minutes)} ${components.minutes === 1 ? "minute" : "minutes"}`,
    );
  }
  if (components.seconds) {
    parts.push(
      `${formatNumber(components.seconds)} ${components.seconds === 1 ? "second" : "seconds"}`,
    );
  }
...
export function addDurationToDate(
  ...
  if (components.years) {
    ...
    if (components.years % 1) {
      result.setTime(
        result.getTime() + (components.years % 1) * 365 * 24 * 60 * 60 * 1000,
      );
    }
  }
  ...
  if (components.months) {
    ...
    if (components.months % 1) {
      result.setTime(
        result.getTime() + (components.months % 1) * 30 * 24 * 60 * 60 * 1000,
      );
    }
  }
  ...
  if (components.weeks) {
    result.setTime(
      result.getTime() + components.weeks * 7 * 24 * 60 * 60 * 1000,
    );
  }
