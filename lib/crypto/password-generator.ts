...
export type PasswordSerialization = "graphic-ascii" | "base64" | "hex" | "base58" | "base45" | "base32"
...
export function generatePassword(options: PasswordGeneratorOptions): PasswordGenerationResult {
  ...
  if (options.serialization === "graphic-ascii") {
    ...
    if (!charset) {
      return { value: "", error: "Select at least one character group or add symbols." }
    }
  ...
  if (options.lengthType === "chars") {
    switch (options.serialization) {
      case "base64": {
        const alphabet = options.base64UrlSafe ? BASE64_URLSAFE : BASE64_STANDARD
        const charset = options.base64NoPadding ? alphabet : alphabet + "="
        return { value: randomFromCharset(length, charset) }
      }
      case "hex": {
        const charset = options.caseMode === "upper" ? HEX_ALPHABET_UPPER : HEX_ALPHABET_LOWER
        return { value: randomFromCharset(length, charset) }
      }
      case "base58":
        return { value: randomFromCharset(length, BASE58_ALPHABET) }
      case "base45": {
        const charset = options.caseMode === "upper" ? BASE45_ALPHABET : BASE45_ALPHABET.toLowerCase()
        return { value: randomFromCharset(length, charset) }
      }
      case "base32": {
        const charset = options.caseMode === "upper" ? BASE32_ALPHABET : BASE32_ALPHABET.toLowerCase()
        return { value: randomFromCharset(length, charset) }
      }
    }
  }
  ...
  switch (options.serialization) {
    case "base64":
      return {
        value: encodeBase64(bytes, { padding: !options.base64NoPadding, urlSafe: options.base64UrlSafe }),
      }
    case "hex":
      return { value: encodeHex(bytes, { upperCase: options.caseMode === "upper" }) }
    case "base58":
      return { value: encodeBase58(bytes) }
    case "base45":
      return { value: encodeBase45(bytes, { upperCase: options.caseMode === "upper" }) }
    case "base32":
      return {
        value: encodeBase32(bytes, { upperCase: options.caseMode === "upper", padding: !options.base32NoPadding }),
      }
    default:
      return { value: "" }
  }
