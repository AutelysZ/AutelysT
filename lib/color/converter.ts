...
function parseHsl(input: string): HSL | null {
  ...
  return { h: ((h % 360) + 360) % 360, s: clamp(s, 0, 100), l: clamp(l, 0, 100), a: clamp(a, 0, 1) }
}
...
function parseHwb(input: string): HWB | null {
  ...
  return { h: ((h % 360) + 360) % 360, w: clamp(w, 0, 100), b: clamp(b, 0, 100), a: clamp(a, 0, 1) }
}
...
function parseLch(input: string): LCH | null {
  ...
  return { l: clamp(l, 0, 100), c: Math.max(0, c), h: ((h % 360) + 360) % 360, a: clamp(a, 0, 1) }
}
...
function parseOklch(input: string): OKLCH | null {
  ...
  return { l: clamp(l, 0, 1), c: Math.max(0, c), h: ((h % 360) + 360) % 360, a: clamp(a, 0, 1) }
}
...
export function rgbToOklab(rgb: RGB): OKLAB {
  ...
  return {
    l: Math.round((0.2104542553 * l + 0.793617785 * m - 0.0040720468 * s) * 1000) / 1000,
    a: Math.round((1.9779984951 * l - 2.428592205 * m + 0.4505937099 * s) * 1000) / 1000,
    b: Math.round((0.0259040371 * l + 0.7827717662 * m - 0.808675766 * s) * 1000) / 1000,
    alpha: rgb.a,
  }
}
