...
export function parseEcPrivateKey(input: string, curve: EcCurveId) {
  ...
  if (jwk) {
    const jwkCurve =
      typeof jwk.crv === "string" ? normalizeEcCurveId(jwk.crv) : null;
    ...
    const privateKey = padBytes(
      decodeBase64Url(jwk.d),
      spec.noble.lengths.secretKey,
    );
  ...
  const privateKey = padBytes(
    decodeHex(key.prvKeyHex),
    spec.noble.lengths.secretKey,
  );
  let publicKey = key.pubKeyHex ? decodeHex(key.pubKeyHex) : null;
  if (!publicKey) {
    publicKey = spec.noble.getPublicKey(
      privateKey,
      false,
...
export function parseDhPrivateKey(input: string) {
  ...
  if (!jwk) {
    throw new Error(
      "DH keys must be provided as JSON with p, g, and x values.",
    );
  }
...
export function createEcPrivateKeyPem(
  curve: EcCurveId,
  privateKey: Uint8Array,
  publicKey?: Uint8Array,
) {
  const spec = getEcCurveSpec(curve);
  const privateKeyInfo = forge.asn1.create(
    ...
    [
      ...
      forge.asn1.create(
        ...
        false,
        forge.asn1
          .toDer(createEcPrivateKeyAsn1(privateKey, publicKey))
          .getBytes(),
      ),
    ],
  );
...
export function createEdPrivateKeyPem(
  curve: EdCurveId,
  privateKey: Uint8Array,
) {
  const privateKeyInfo = forge.asn1.create(
      ...
      forge.asn1.create(
        ...
        [
          ...
          forge.asn1.create(
            ...
            forge.asn1.Type.SEQUENCE,
            true,
            [asn1IntegerFromBigInt(params.p), asn1IntegerFromBigInt(params.g)],
          ),
        ],
      ),
      forge.asn1.create(
...
export function publicKeyToAsn1(key: CertPublicKey) {
  if (key.type === "rsa") {
    const rsaPublicKey = forge.asn1.create(
      forge.asn1.Class.UNIVERSAL,
      forge.asn1.Type.SEQUENCE,
      true,
      [asn1IntegerFromBigInt(key.n), asn1IntegerFromBigInt(key.e)],
    );
        ...
        forge.asn1.create(
          ...
          [
            forge.asn1.create(
              ...
              false,
              forge.asn1
                .oidToDer(ED_OIDS[key.type === "ed25519" ? "Ed25519" : "Ed448"])
                .getBytes(),
            ),
          ],
        ),
  ...
  if (key.type === "dsa") {
    ...
    const publicInt = forge.asn1
      .toDer(asn1IntegerFromBigInt(params.y))
      .getBytes();
    return forge.asn1.create(
      forge.asn1.Class.UNIVERSAL,
      forge.asn1.Type.SEQUENCE,
...
function createEcPrivateKeyAsn1(
  privateKey: Uint8Array,
  publicKey?: Uint8Array,
) {
  const items: any[] = [
