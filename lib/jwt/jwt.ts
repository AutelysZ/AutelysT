...
export function parseJwt(token: string): {
  ...
  if (parts.length !== 3)
    return { error: "Token must have three parts separated by dots." };
...
export function decodeSecret(
  ...
  if (encoding === "utf8")
    return textEncoder.encode(secret) as Uint8Array<ArrayBuffer>;
  if (encoding === "base64") {
    try {
...
async function importAsymmetricKey(
  ...
  if (jwk) {
    ...
    if (isRsaAlg(alg)) {
      return crypto.subtle.importKey(
        "jwk",
        jwk,
        { name: "RSASSA-PKCS1-v1_5", hash: { name: getRsaHash(alg) } },
        false,
    ...
    if (isRsaPssAlg(alg)) {
      return crypto.subtle.importKey(
        "jwk",
        jwk,
        { name: "RSA-PSS", hash: { name: getRsaHash(alg) } },
        false,
        [mode === "sign" ? "sign" : "verify"],
      );
    }
    if (isEcdsaAlg(alg)) {
      return crypto.subtle.importKey(
        "jwk",
        jwk,
        { name: "ECDSA", namedCurve: getEcdsaCurve(alg) },
        false,
        [mode === "sign" ? "sign" : "verify"],
      );
    }
  ...
  if (
    mode === "verify" &&
    !parsed.label.includes("PUBLIC KEY") &&
...
export async function signJwtWithKey({
  ...
  if (isRsaPssAlg(alg)) {
    const signature = await crypto.subtle.sign(
      {
        name: "RSA-PSS",
        saltLength: alg === "PS256" ? 32 : alg === "PS384" ? 48 : 64,
      },
      key,
      textEncoder.encode(signingInput),
    );
