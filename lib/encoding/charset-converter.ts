  ...
  switch (encoding) {
    case "raw": {
      if (!isUtf8Charset(charset)) {
        throw new Error(
          "Raw input encoding is only supported for UTF-8 charset",
        );
      }
...
export function bytesToDisplayText(
  bytes: Uint8Array,
...
export function encodeOutput(
  ...
  switch (encoding) {
    ...
    case "url": {
      ...
      if (
        outputCharset !== "UTF-8" &&
        encoded.includes("%EF%BB%BF") === false &&
        bytes.length > 0
      ) {
        return encodeUrlBytes(bytes);
      }
...
export function convertCharset(
  ...
  const { inputEncoding, outputEncoding, inputCharset, outputCharset } =
    options;
  ...
  if (inputCharset.toUpperCase() === outputCharset.toUpperCase()) {
    ...
    outputBytes = convertBytesCharset(
      decodedBytes,
      inputCharset,
      outputCharset,
    );
...
export function convertBytesCharset(
  ...
  if (toLower === "utf-8") {
    ...
    codePoints.forEach((cp, i) =>
      view.setUint32(i * 4, cp, toLower === "utf-32le"),
    );
    return new Uint8Array(buf);
  } else {
    try {
  ...
  if (outputEncoding === "raw") {
    ...
    try {
      ...
      const buffer = iconv.encode(
        new TextDecoder(charset.toLowerCase()).decode(bytes),
        charset.toLowerCase(),
      );
      return {
        content: buffer as Uint8Array,
...
function detectBOM(
  bytes: Uint8Array,
): { charset: string; consumed: number } | null {
...
function isTextContent(bytes: Uint8Array): boolean {
  const textChars = new Set([
    0x09, 0x0a, 0x0d, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28,
    ...
    0x7d, 0x7e, 0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, 0xa7, 0xa8, 0xa9,
