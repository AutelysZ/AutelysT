          ...
          messages.push({
            ...
            package:
              type instanceof protobuf.Namespace
                ? (type as protobuf.Namespace).fullName
                : undefined,
          });
      ...
      for (const [name, nested] of Object.entries(type.nested)) {
        ...
        if (nested instanceof protobuf.Enum) {
          enums.push({
            name,
            fullName,
            values: Object.entries(nested.values).map(([n, v]) => ({
              name: n,
              number: v,
            })),
          });
  ...
  while (offset < data.length) {
    ...
    switch (wireType) {
      ...
      case WIRE_TYPES.FIXED64: {
        ...
        // Signed fixed64
        const sfixed64 =
          uint64 > 0x7fffffffffffffffn ? uint64 - 0x10000000000000000n : uint64;
        field.interpretations.push({
          type: "sfixed64",
...
function fieldsToObject(fields: DecodedField[]): Record<string, unknown> {
  ...
  for (const field of fields) {
    ...
    if (field.interpretations.length > 0) {
      ...
      const embeddedInterp = field.interpretations.find(
        (i) => i.type === "embedded message",
      );
...
export function formatFieldValue(field: DecodedField): string {
  if (field.interpretations.length > 0) {
    ...
    if (typeof best.value === "string") {
      return best.value.length > 100
        ? best.value.substring(0, 100) + "..."
        : best.value;
    }
...
function encodeFieldValue(
  ...
  switch (type) {
    ...
    case "uint64": {
      const num =
        typeof value === "string" ? BigInt(value) : BigInt(value as number);
...
export function generateProtoFromDecodedFields(fields: DecodedField[]): string {
  ...
  for (const field of fields) {
    ...
    if (highConfidence) {
      switch (highConfidence.type) {
        ...
        case "uint32":
          inferredType =
            field.wireType === WIRE_TYPES.VARINT ? "int64" : "fixed64";
          break;
        case "fixed64":
  ...
  const sortedFields = Array.from(fieldDefs.entries()).sort(
    (a, b) => a[0] - b[0],
  );
...
export function generateProtoFromObject(
  ...
  function inferType(
    ...
    if (typeof value === "object") {
      ...
      const nestedProto = generateNestedMessage(
  ...
  function generateNestedMessage(
    ...
    for (const [key, value] of Object.entries(nestedObj)) {
      ...
      nestedLines.push(
        `  ${repeatedPrefix}${type} ${key} = ${nestedFieldNumber};`,
      );
      nestedFieldNumber++;
    }
...
export function decodeWithFieldTable(
  ...
  for (const rawField of rawFields) {
    ...
    switch (fieldType) {
      case "string": {
        const strInterp = rawField.interpretations.find(
          (i) => i.type === "string",
        );
      ...
      case "bool": {
        const boolInterp = rawField.interpretations.find(
          (i) => i.type === "bool",
        );
        value =
          boolInterp?.value ??
          (rawField.rawValue === "1" || rawField.rawValue === 1);
        break;
      }
      ...
      case "float": {
        const numInterp = rawField.interpretations.find(
          (i) =>
            i.type === "uint64" || i.type === "fixed32" || i.type === "float",
        );
      ...
      case "double": {
        const numInterp = rawField.interpretations.find(
          (i) =>
            i.type === "uint64" || i.type === "fixed64" || i.type === "double",
        );
        // Keep as string for 64-bit values to preserve precision
        value =
          numInterp?.value !== undefined
            ? String(numInterp.value)
            : String(rawField.rawValue);
        break;
      }

      case "sint32":
      case "sint64": {
        const sintInterp = rawField.interpretations.find((i) =>
          i.type.includes("zigzag"),
        );
      ...
      case "bytes": {
        const bytesInterp = rawField.interpretations.find(
          (i) => i.type === "bytes",
        );
