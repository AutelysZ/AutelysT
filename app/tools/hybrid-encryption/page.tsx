...
const hpkeKemMap: Record<HpkeKem, () => DhkemP256HkdfSha256 | DhkemP384HkdfSha384 | DhkemP521HkdfSha512> = {
...
async function importHpkeKey(keyText: string, type: "public" | "private", kem: HpkeKem) {
...
function HybridEncryptionContent() {
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      ;(Object.keys(paramsSchema.shape) as (keyof HybridState)[]).forEach((key) => {
...
function HybridEncryptionInner({
  ...
  state: HybridState
  setParam: <K extends keyof HybridState>(key: K, value: HybridState[K], immediate?: boolean) => void
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const preview = signature.slice(0, 100) || `${state.standard} ${state.mode}`
      upsertInputEntry({ input: state.input }, paramsSnapshot(), "left", preview)
    }, DEFAULT_URL_SYNC_DEBOUNCE_MS)

    return () => clearTimeout(timer)
  }, [state.input, state.standard, state.mode, upsertInputEntry, paramsSnapshot])
  ...
  React.useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (isCms) {
          ...
          if (isEncrypt) {
            if (!state.cmsRecipientCert.trim()) {
              throw new Error("Recipient certificate is required for CMS encryption.")
            }
            ...
            if (!state.cmsRecipientKey.trim()) {
              throw new Error("Recipient private key is required for CMS decryption.")
            }
            const privateKey = forge.pki.privateKeyFromPem(state.cmsRecipientKey)
            const input = state.input.trim()
            const message = isPemBlock(input)
              ? forge.pkcs7.messageFromPem(input)
              : forge.pkcs7.messageFromAsn1(forge.asn1.fromDer(forge.util.decode64(input)))
          ...
          if (isEncrypt) {
            ...
            const publicKey = await openpgp.readKey({ armoredKey: state.pgpPublicKey })
            const message = await openpgp.createMessage({ text: state.input })
            nextOutput = await openpgp.encrypt({ message, encryptionKeys: publicKey, format: "armored" })
          } else {
            if (!state.pgpPrivateKey.trim()) {
              throw new Error("Private key is required for OpenPGP decryption.")
            }
            const message = await openpgp.readMessage({ armoredMessage: state.input })
            const privateKey = await openpgp.readPrivateKey({ armoredKey: state.pgpPrivateKey })
            const decryptionKey = state.pgpPassphrase
              ? await openpgp.decryptKey({ privateKey, passphrase: state.pgpPassphrase })
              : privateKey
            const { data } = await openpgp.decrypt({ message, decryptionKeys: decryptionKey, format: "utf8" })
          ...
          if (state.jweAlg === "dir") {
            ...
            if (keyBytes.length !== 32) {
              throw new Error("Direct JWE key must be 32 bytes (256 bits) for A256GCM.")
            }
            if (isEncrypt) {
              nextOutput = await new jose.CompactEncrypt(textEncoder.encode(state.input))
                .setProtectedHeader({ alg: "dir", enc: "A256GCM" })
                .encrypt(keyBytes)
            } else {
              const { plaintext } = await jose.compactDecrypt(state.input, keyBytes)
            ...
            if (!isEncrypt && !state.jwePrivateKey.trim()) {
              throw new Error("Private key is required for JWE RSA decryption.")
            }
            const keyText = isEncrypt ? state.jwePublicKey : state.jwePrivateKey
            const jwk = parseJwk(keyText)
            const key = jwk
          ...
          if (isEncrypt) {
            if (!state.hpkePublicKey.trim()) {
              throw new Error("Recipient public key is required for HPKE encryption.")
            }
            ...
            if (!state.hpkePrivateKey.trim()) {
              throw new Error("Recipient private key is required for HPKE decryption.")
            }
        ...
        if (runRef.current !== runId) return
        setError(err instanceof Error ? err.message : "Failed to process hybrid encryption.")
        setOutput("")
        setHpkeEncOutput("")
      } finally {
  ...
  const handleGenerateHpkeKeypair = async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to generate HPKE keypair.")
    } finally {
  ...
  const inputPlaceholder = isEncrypt ? "Enter text to encrypt..." : "Paste ciphertext to decrypt..."
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <Tabs
              ...
              onValueChange={(value) => setParam("standard", value as StandardValue, true)}
            ...
            <div className="space-y-3">
              ...
              {isEncrypt ? (
                <div className="flex items-start gap-3">
                  <Label className="w-24 text-sm sm:w-32 pt-2">Recipient Cert</Label>
                  <div className="min-w-0 flex-1">
                    <Textarea
                      value={state.cmsRecipientCert}
                      onChange={(event) => setParam("cmsRecipientCert", event.target.value)}
                ...
                <div className="flex items-start gap-3">
                  <Label className="w-24 text-sm sm:w-32 pt-2">Private Key</Label>
                  <div className="min-w-0 flex-1">
                    <Textarea
                      value={state.cmsRecipientKey}
                      onChange={(event) => setParam("cmsRecipientKey", event.target.value)}
          ...
          {isOpenPgp && (
            <div className="space-y-3">
              {isEncrypt ? (
                <div className="flex items-start gap-3">
                  <Label className="w-24 text-sm sm:w-32 pt-2">Public Key</Label>
                  <div className="min-w-0 flex-1">
                    <Textarea
                      value={state.pgpPublicKey}
                      onChange={(event) => setParam("pgpPublicKey", event.target.value)}
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <Textarea
                        ...
                        onChange={(event) => setParam("pgpPrivateKey", event.target.value)}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Input
                      ...
                      onChange={(event) => setParam("pgpPassphrase", event.target.value)}
          ...
          {isJwe && (
            <div className="space-y-3">
              ...
              {state.jweAlg === "dir" ? (
                <div className="flex items-start gap-3">
                  <Label className="w-24 text-sm sm:w-32 pt-2">Shared Key</Label>
                  <div className="min-w-0 flex-1">
                    <Textarea
                      value={state.jweKey}
                      onChange={(event) => setParam("jweKey", event.target.value)}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("jwePublicKey", event.target.value)}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("jwePrivateKey", event.target.value)}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("hpkePublicKey", event.target.value)}
                      placeholder="JWK (EC)"
                      className={cn(
                        "min-h-[140px] max-h-[220px] overflow-auto break-all font-mono text-xs",
                        oversizeKeys.includes("hpkePublicKey") && "border-destructive",
                      )}
                    />
                    <div className="mt-2 flex items-center justify-between gap-2">
                      <p className="text-xs text-muted-foreground">EC JWK only.</p>
                      <Button
                        variant="ghost"
                        size="sm"
                ...
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <Textarea
                        ...
                        onChange={(event) => setParam("hpkePrivateKey", event.target.value)}
                        placeholder="JWK (EC)"
                        className={cn(
                          "min-h-[140px] max-h-[220px] overflow-auto break-all font-mono text-xs",
                          oversizeKeys.includes("hpkePrivateKey") && "border-destructive",
                        )}
                      />
                      <div className="mt-2 flex items-center justify-between gap-2">
                        <p className="text-xs text-muted-foreground">EC JWK only.</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <Textarea
                        value={state.hpkeEnc}
                        onChange={(event) => setParam("hpkeEnc", event.target.value)}
                        placeholder="Encapsulated key (same encoding as ciphertext)"
                        className={cn(
                          "min-h-[80px] max-h-[140px] overflow-auto break-all font-mono text-xs",
                          oversizeKeys.includes("hpkeEnc") && "border-destructive",
                        )}
                      />
                      {oversizeKeys.includes("hpkeEnc") && (
                        <p className="text-xs text-muted-foreground">Enc exceeds 2 KB and is not synced to the URL.</p>
                      )}
                    </div>
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex shrink-0 items-center gap-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => handleDownload(output, "hybrid-encryption-output.txt")}
                  className="h-7 w-7 p-0"
            ...
            <Textarea
              value={output}
              readOnly
              placeholder={isWorking ? "Working..." : "Output will appear here..."}
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <div className="flex shrink-0 items-center gap-1">
                  ...
                  >
                    {copied === "hpkeEnc" ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                  </Button>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => handleDownload(hpkeEncOutput, "hybrid-encryption-enc.txt")}
                    className="h-7 w-7 p-0"
