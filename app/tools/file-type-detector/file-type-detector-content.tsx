...
import type { DetectedFileType, FileRecord, FileTreeNode, PreviewState } from "./file-type-detector-types"
...
function buildFileTree(records: FileRecord[]): FileTreeNode[] {
  ...
  for (const record of records) {
    ...
    segments.forEach((segment, index) => {
      ...
      let next = cursor.find((node) => node.type === "directory" && node.name === segment)
      if (!next) {
        next = {
          id: `dir:${currentPath}`,
          name: segment,
...
function isHeicType(mime: string, ext: string) {
  return mime === "image/heic" || mime === "image/heif" || HEIC_EXTENSIONS.has(ext)
}

function getPreviewKind(mime: string, fileName: string, detectedExt?: string): PreviewState["kind"] {
...
export default function FileTypeDetectorContent() {
  ...
  const activeFile = React.useMemo(() => files.find((item) => item.id === activeFileId) ?? null, [files, activeFileId])
  ...
  const processFiles = React.useCallback(
    async (incoming: File[], mode: "append" | "replace") => {
      ...
      setFiles((prev) => (mode === "replace" ? records : [...prev, ...records]))
  ...
  React.useEffect(() => {
    ...
    if (previewKind === "text") {
      readTextPreview(file)
        .then((textPreview) => {
          if (previewIdRef.current !== previewId) return
          applyPreview({ kind: "text", content: textPreview.content, truncated: textPreview.truncated })
        })
        .catch((err) => {
    ...
    if (previewKind === "image") {
      if (isHeicType(mime, ext)) {
        import("heic2any")
          .then((module) => module.default({ blob: file, toType: "image/jpeg" }))
          .then((result) => {
