...
function HtmlRendererContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } =
    useUrlSyncedState("html-renderer", {
      schema: paramsSchema,
  ...
  const [htmlType, setHtmlType] = React.useState<"full" | "snippet">(() =>
    detectHtmlType(defaultHtml),
  );
  ...
  const [consoleFilter, setConsoleFilter] =
    React.useState<ConsoleFilter>("all");
  const [showConsoleTimestamps, setShowConsoleTimestamps] =
    React.useState(true);
  ...
  const appendConsoleMessage = React.useCallback(
    (incoming: Omit<ConsoleMessage, "id" | "count">) => {
  ...
  React.useEffect(() => {
    ...
    if (htmlType === "snippet") {
      const fullContent = injectConsoleScript(
        `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
            ...
            <div className="flex-1 p-4">
              <div className="flex h-full flex-col overflow-hidden rounded-lg border bg-background">
                ...
                <ConsolePanel
                  ...
                  onToggleTimestamps={() =>
                    setShowConsoleTimestamps(!showConsoleTimestamps)
                  }
...
function HtmlRendererInner({
  ...
  React.useEffect(() => {
    if (!state.htmlContent || state.htmlContent === lastInputRef.current)
      return;
        ...
        <div className="flex w-full flex-1 flex-col gap-2 md:w-0">
          ...
          {oversizeKeys.includes("htmlContent") && (
            <p className="text-xs text-muted-foreground">
              HTML content exceeds 2 KB and is not synced to URL.
            </p>
          )}
        </div>
...
function ConsolePanel({
  ...
  const countBadgeClass = isCompact
    ? "text-[9px] px-1.5 py-0.5"
    : "text-[10px] px-2 py-0.5";
  const toolbarButtonClass = isCompact
    ? "h-6 px-2 text-[10px]"
    : "h-7 px-2 text-xs";
  ...
  const renderMessage = (msg: ConsoleMessage) => {
    ...
    const levelLabel =
      msg.type === "error" ? "Error" : msg.type === "warn" ? "Warn" : "Log";
    const levelClass =
      msg.type === "error"
    ...
    const messageText = (
      <div className="flex-1 min-w-0">
        <span
          className={cn("block font-mono whitespace-pre-wrap", levelTextClass)}
        >
          {msg.message}
        </span>
      </div>
    ...
    return (
      <div key={msg.id} className={cn("flex items-start gap-2", rowTextClass)}>
        {showTimestamps && (
          <span
            className={cn("text-muted-foreground shrink-0", timestampClass)}
          >
            {timeLabel}
          </span>
        )}
          ...
          <details className="group flex-1 min-w-0">
            <summary className="flex items-start gap-2 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
              <span className="mt-0.5 text-muted-foreground">
                <ChevronRight
                  className={cn(iconSizeClass, "group-open:hidden")}
                />
                <ChevronDown
                  className={cn(iconSizeClass, "hidden group-open:inline")}
                />
              </span>
              {messageText}
              {countBadge}
      ...
      >
        <div className="flex items-center gap-2">
          ...
          {(counts.error > 0 || counts.warn > 0) && (
            <AlertTriangle className={cn(iconSizeClass, "text-destructive")} />
          )}
          {counts.total > 0 && (
            <span
              className={cn(
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {filteredMessages.length === 0 ? (
              <div
                className={cn(emptyTextClass, "text-muted-foreground italic")}
              >
