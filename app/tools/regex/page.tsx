...
function HighlightedTestString({
  ...
  for (const match of sortedMatches) {
    ...
    if (match.index > lastEnd) {
      segments.push(
        <span key={`t-${lastEnd}`}>
          {testString.slice(lastEnd, match.index)}
        </span>,
      );
    }
    segments.push(
      <span
        key={`m-${match.index}`}
        className="bg-yellow-300 dark:bg-yellow-700 rounded-sm px-0.5"
      >
        {match.value}
      </span>,
    );
  ...
  if (lastEnd < testString.length) {
    segments.push(
      <span key={`t-${lastEnd}`}>{testString.slice(lastEnd)}</span>,
    );
  }
...
function FlavorConversionRow({
  ...
  return (
    <div
      className={`group flex items-start gap-3 p-2 rounded-lg hover:bg-muted/30 transition-colors ${hasIssues ? "bg-destructive/5" : ""}`}
    >
...
function RegexContent() {
  ...
  const [results, setResults] = useState<
    Record<string, RegexTestResult | null>
  >({});
  ...
  const ecmascriptConversion = useMemo(() => {
    ...
    if (inputFlavor === "ecmascript") {
      return {
        output: fullPattern,
        outputPattern: parsedInput.pattern,
        outputFlags: parsedInput.flags || "g",
        warnings: [],
        changes: [],
      };
    }
  ...
  const testPattern =
    ecmascriptConversion?.outputPattern || parsedInput.pattern;
  ...
  const updateTestString = useCallback((id: string, value: string) => {
    setTestStrings((prev) =>
      prev.map((ts) => (ts.id === id ? { ...ts, value } : ts)),
    );
  }, []);
              ...
              <div className="space-y-3">
                ...
                <div className="relative">
                  <Textarea
                    ...
                    placeholder={
                      inputFlavor === "ecmascript"
                        ? "/pattern/flags"
                        : inputFlavor === "python"
                          ? '(?flags)pattern or r"pattern"'
                          : "(?flags)pattern"
                    }
                    ...
                    <div className="mt-2 pt-3 border-t space-y-4">
                      ...
                      <div className="space-y-2 text-xs">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {flavorInfo.flags.map((f) => (
                              ...
                              >
                                {FLAG_NAMES[f] || f}{" "}
                                <code className="ml-0.5 opacity-60">({f})</code>
                              </Badge>
                            ))}
                          </div>
                        </div>
                        ...
                        {(() => {
                          ...
                          const unsupportedFlags = ALL_FLAGS.filter(
                            (f) => !flavorInfo.flags.includes(f),
                          );
                            ...
                            <div className="flex flex-wrap gap-x-4 gap-y-1">
                              ...
                              <div className="flex flex-wrap gap-1">
                                {unsupportedFlags.map((f) => (
                                  ...
                                  >
                                    {FLAG_NAMES[f] || f}{" "}
                                    <code className="ml-0.5 opacity-60">
                                      ({f})
                                    </code>
                                  </Badge>
                                ))}
                ...
                fullPattern.trim() && (
                  <div className="mb-3 p-2 bg-blue-50 dark:bg-blue-950/30 rounded text-xs flex items-center gap-2">
                    <Badge variant="outline" className="text-xs">
                      Testing as ECMAScript
                    </Badge>
                    <code className="font-mono text-blue-600 dark:text-blue-400 truncate">
                      {ecmascriptConversion.output}
                    </code>
                  </div>
                )}
