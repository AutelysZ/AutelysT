...
function buildLayout(config: SnowflakeConfig): SnowflakeLayout {
  ...
  const timestampShift = BigInt(
    config.sequenceBits + config.workerBits + config.datacenterBits,
  );
...
function buildSnowflakeId(
  ...
  const timePart =
    (timestampOffset & layout.timestampMask) << layout.timestampShift;
  const datacenterPart =
    (datacenterId & layout.datacenterMask) << layout.datacenterShift;
...
function parseSnowflakeId(
  ...
  const timestampMsBig =
    BigInt(config.startTimeMs) +
    timestampOffset * BigInt(Math.max(config.timestampOffsetMs, 1));
  ...
  return {
    ...
    timestampOffsetMs: (
      timestampOffset * BigInt(config.timestampOffsetMs)
    ).toString(),
    datacenterId: datacenterId.toString(),
    workerId: workerId.toString(),
    sequence: sequence.toString(),
    overflow: overflowReasons.length > 0,
    overflowReason: overflowReasons.length
      ? overflowReasons.join("; ")
      : undefined,
  };
}
...
function SnowflakeIdContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } =
    useUrlSyncedState("snowflake-id", {
      schema: paramsSchema,
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      if (params.datacenterId !== undefined)
        setParam("datacenterId", params.datacenterId as number);
      if (params.workerId !== undefined)
        setParam("workerId", params.workerId as number);
      if (params.timestampBits !== undefined)
        setParam("timestampBits", params.timestampBits as number);
      if (params.datacenterBits !== undefined)
        setParam("datacenterBits", params.datacenterBits as number);
      if (params.workerBits !== undefined)
        setParam("workerBits", params.workerBits as number);
      if (params.sequenceBits !== undefined)
        setParam("sequenceBits", params.sequenceBits as number);
      if (params.startTimeMs !== undefined)
        setParam("startTimeMs", params.startTimeMs as number);
      if (params.timestampOffsetMs !== undefined)
        setParam("timestampOffsetMs", params.timestampOffsetMs as number);
    },
    [setParam],
  );
...
function SnowflakeIdInner({
  ...
  const layout = React.useMemo(() => buildLayout(config), [config]);
  const datacenterMax = React.useMemo(
    () => bigIntToSafeNumber(layout.datacenterMask),
    [layout.datacenterMask],
  );
  const workerMax = React.useMemo(
    () => bigIntToSafeNumber(layout.workerMask),
    [layout.workerMask],
  );
  const presetMatch = React.useMemo(
    () => matchPreset(state),
    [
      state.timestampBits,
      state.datacenterBits,
  ...
  const paramWarnings = React.useMemo(() => {
    ...
    if (layout.totalBits > MAX_BITS) {
      warnings.push(
        `Total bits (${layout.totalBits}) exceed ${MAX_BITS}-bit Snowflake IDs.`,
      );
    }
    ...
    if (BigInt(state.workerId) > layout.workerMask) {
      warnings.push(
        `Worker ID exceeds ${state.workerBits}-bit max (${layout.workerMask.toString()}).`,
      );
    }
    const now = BigInt(Date.now());
    const maxTimestampMs =
      BigInt(state.startTimeMs) +
      layout.timestampMask * BigInt(state.timestampOffsetMs);
  ...
  const handleGenerate = React.useCallback(() => {
    try {
      const ids = generateSnowflakeIds(
        state.count,
        state.datacenterId,
        state.workerId,
        config,
      ...
      setGenerationError(
        error instanceof Error
          ? error.message
          : "Unable to generate Snowflake IDs.",
      );
    }
  }, [
    state.count,
  ...
  const handlePresetChange = React.useCallback(
    (presetId: SnowflakePreset["id"]) => {
      const preset =
        SNOWFLAKE_PRESETS.find((item) => item.id === presetId) ??
        DEFAULT_PRESET;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-xs text-muted-foreground">
        Start time: {startTimeIso}. Timestamp offset: {state.timestampOffsetMs}{" "}
        ms per tick.
      </p>
            ...
            hasMultipleParsed ? (
              ...
              <Card>
                <CardContent className="space-y-3 p-4">
                  ...
                  <InfoRow
                    label="Timestamp (ISO)"
                    value={singleParsed.timestampIso}
                  />
                  <InfoRow
                    label="Timestamp (ms)"
                    value={String(singleParsed.timestampMs)}
                    mono
                  />
                  <InfoRow
                    label="Timestamp Offset (ticks)"
                    value={singleParsed.timestampOffsetTicks}
                    mono
                  />
                  <InfoRow
                    label="Timestamp Offset (ms)"
                    value={singleParsed.timestampOffsetMs}
                    mono
                  />
                  <InfoRow
                    label="Datacenter ID"
                    value={singleParsed.datacenterId}
                    mono
                  />
                  <InfoRow
                    label="Worker ID"
                    value={singleParsed.workerId}
                    mono
                  />
                  <InfoRow
                    label="Sequence"
                    value={singleParsed.sequence}
                    mono
                  />
                  <InfoRow
                    label="Overflow"
                    value={
                      singleParsed.overflow
                        ? (singleParsed.overflowReason ?? "Yes")
                        : "No"
                    }
                  />
                </CardContent>
              ...
              <Card className="flex min-h-[300px] items-center justify-center">
                <p className="text-sm text-destructive">
                  {singleParsedError ?? "Snowflake ID is invalid."}
                </p>
              </Card>
            )
          ) : (
