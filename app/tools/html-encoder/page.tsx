...
function HtmlEncoderContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } = useUrlSyncedState("html-encoder", {
    schema: paramsSchema,
  ...
  const encodeToHtml = React.useCallback(
    (text: string) => {
      try {
        ...
        setLeftError(error instanceof Error ? error.message : "Encoding failed")
      }
    },
  ...
  const decodeFromHtml = React.useCallback(
    (text: string) => {
      try {
        ...
        setRightError(error instanceof Error ? error.message : "Decoding failed")
      }
    },
  ...
  }, [state.activeSide, state.leftText, state.rightText, encodeToHtml, decodeFromHtml])
