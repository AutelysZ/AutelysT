...
export function decodeKeyBytes(
  value: string,
...
export function encodeSignatureBytes(
  bytes: Uint8Array,
...
export function decodeSignatureBytes(
  value: string,
  encoding: SignatureEncoding,
): Uint8Array<ArrayBuffer> {
  if (!value) return new Uint8Array();
...
export function decodePqcKeyBytes(
  value: string,
...
export function resolvePqcKeyBytes(
  keyText: string,
  encoding: PqcKeyEncoding,
  type: "public" | "private",
) {
...
export function createPqcPublicKey(
  algorithm: string,
  publicKey: Uint8Array,
  encoding: PqcKeyEncoding,
) {
...
export async function importPemAsJwk({
  ...
  if (format === "private" && !parsed.label.includes("PRIVATE KEY"))
    return null;
...
export async function resolveEcJwk(
  keyText: string,
  curve: EcdsaCurve,
  usage: "sign" | "verify",
) {
  ...
  return importPemAsJwk({
    keyText,
    algorithm,
    format: usage === "sign" ? "private" : "public",
    usages,
  });
}

export async function resolveOkpJwk(
  keyText: string,
  curve: EddsaCurve,
  usage: "sign" | "verify",
) {
...
export async function getEcdsaPrivateKeyBytes(
  keyText: string,
  curve: EcdsaCurve,
) {
  ...
  if (!jwk) {
    throw new Error(
      "Invalid EC key format. Use JWK or PEM (P-256/P-384/P-521).",
    );
  }
  if (!("d" in jwk)) {
...
export async function getEcdsaPublicKeyBytes(
  keyText: string,
  curve: EcdsaCurve,
) {
...
export async function getEddsaPrivateKeyBytes(
  keyText: string,
  curve: EddsaCurve,
) {
...
export async function getEddsaPublicKeyBytes(
  keyText: string,
  curve: EddsaCurve,
) {
...
export async function hashMessageBytes(
