...
const registeredClaims = ["iss", "sub", "aud", "exp", "nbf", "iat", "jti"] as const
...
function JwtContent() {
  const { state, setParam, setState, oversizeKeys, hasUrlParams, urlParamKeys, hydrationSource } = useUrlSyncedState(
    "jwt",
    {
      schema: paramsSchema,
      defaults: paramsSchema.parse({}),
      shouldSyncParam: (key, _value, current) => {
        if (key === "secretText" || key === "secretEncoding" || key === "secretPublicText" || key === "secretPrivateText")
          return false
  ...
  const [headerExtraError, setHeaderExtraError] = React.useState<string | null>(null)
  ...
  const [signatureStatus, setSignatureStatus] = React.useState<{ status: string; message: string } | null>(null)
  ...
  const parsedTokenRef = React.useRef<ReturnType<typeof parseJwt>["parsed"] | null>(null)
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      if (inputs.tokenText !== undefined) setParam("tokenText", inputs.tokenText)
      ...
      if (params.headerExtraJson !== undefined) setParam("headerExtraJson", params.headerExtraJson as string)
      if (params.headerView) setParam("headerView", params.headerView as JwtState["headerView"])
      if (params.iss !== undefined) setParam("iss", params.iss as string)
      if (params.sub !== undefined) setParam("sub", params.sub as string)
      if (params.audIsArray !== undefined) setParam("audIsArray", params.audIsArray as boolean)
      if (params.audText !== undefined) setParam("audText", params.audText as string)
      if (params.audListText !== undefined) setParam("audListText", params.audListText as string)
      if (params.expText !== undefined) setParam("expText", params.expText as string)
      if (params.nbfText !== undefined) setParam("nbfText", params.nbfText as string)
      if (params.iatText !== undefined) setParam("iatText", params.iatText as string)
      if (params.jti !== undefined) setParam("jti", params.jti as string)
      if (params.extraJsonText !== undefined) setParam("extraJsonText", params.extraJsonText as string)
      if (params.payloadView) setParam("payloadView", params.payloadView as JwtState["payloadView"])
      if (params.secretText !== undefined) setParam("secretText", params.secretText as string)
      if (params.secretPublicText !== undefined) setParam("secretPublicText", params.secretPublicText as string)
      if (params.secretPrivateText !== undefined) setParam("secretPrivateText", params.secretPrivateText as string)
      if (params.secretEncoding) setParam("secretEncoding", params.secretEncoding as JwtState["secretEncoding"])
    },
    [setParam],
  )
  ...
  React.useEffect(() => {
    ...
    const headerExtras = Object.fromEntries(
      Object.entries(header).filter(([key]) => !["alg", "typ", "kid"].includes(key)),
    )
    const headerExtraJson = Object.keys(headerExtras).length ? JSON.stringify(headerExtras, null, 2) : ""
    ...
    const extraClaims = Object.fromEntries(
      Object.entries(payload).filter(([key]) => !registeredClaims.includes(key as RegisteredClaim)),
    )
    const extraJsonText = Object.keys(extraClaims).length ? JSON.stringify(extraClaims, null, 2) : ""
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      setHeaderExtraError(error instanceof Error ? error.message : "Invalid JSON.")
    }
  ...
  React.useEffect(() => {
    const run = async () => {
      ...
      if (!parsedTokenRef.current) {
        setSignatureStatus({ status: "Invalid", message: "Token parsing failed." })
        return
      }

      const headerAlg = parsedTokenRef.current.header.alg
      const alg = typeof headerAlg === "string" ? (headerAlg as JwtAlg) : "none"
      if (!algOptions.includes(alg)) {
        setSignatureStatus({ status: "Unsupported", message: "Unsupported algorithm." })
        return
      }

      if (alg === "none") {
        const ok = parsedTokenRef.current.signature.length === 0
        setSignatureStatus({ status: ok ? "Valid" : "Invalid", message: ok ? "Unsigned token." : "Unexpected signature." })
        return
      }

      const keyForVerify = alg.startsWith("HS") ? state.secretText : state.secretPublicText || state.secretPrivateText
      if (!keyForVerify) {
        setSignatureStatus({
          status: "Unknown",
          message: alg.startsWith("HS") ? "Secret is required for verification." : "Public key is required for verification.",
        })
        return
      }

      if (!crypto?.subtle) {
        setSignatureStatus({ status: "Unsupported", message: "Web Crypto unavailable." })
        return
      }
      ...
      try {
        ...
        if (ok === null) {
          setSignatureStatus({ status: "Invalid", message: "Key decoding failed." })
          return
        }
        setSignatureStatus({ status: ok ? "Valid" : "Invalid", message: ok ? "Signature matches." : "Signature mismatch." })
      } catch (error) {
        setSignatureStatus({
          status: "Invalid",
    ...
    void run()
  }, [state.tokenText, state.secretText, state.secretPublicText, state.secretPrivateText, state.secretEncoding])
  ...
  const handleGenerate = React.useCallback(async () => {
    ...
    if (headerExtra) {
      try {
        ...
        errors.push(error instanceof Error ? error.message : "Invalid header JSON.")
      }
    }
    ...
    if (extraJson) {
      try {
        ...
        errors.push(error instanceof Error ? error.message : "Invalid extra JSON.")
      }
    }

    const conflictKeys = Object.keys(extraObj).filter((key) => registeredClaims.includes(key as RegisteredClaim))
    if (conflictKeys.length > 0) {
      errors.push(`Extra JSON includes registered claims: ${conflictKeys.join(", ")}`)
    }

    const timeClaims: Array<{ key: "expText" | "nbfText" | "iatText"; label: string }> = [
    ...
    if (alg !== "none") {
      const signed = await signJwtWithKey({
        alg,
        signingInput,
        secret: alg.startsWith("HS") ? state.secretText : state.secretPrivateText,
        encoding: state.secretEncoding,
      })
      if (signed === null) {
  ...
  const tokenWarning = oversizeKeys.includes("tokenText") ? "Token exceeds 2 KB and is not synced to the URL." : null
...
function JwtInner({
  ...
  state: JwtState
  setParam: <K extends keyof JwtState>(key: K, value: JwtState[K], updateHistory?: boolean) => void
  setState: (updater: JwtState | ((prev: JwtState) => JwtState), immediate?: boolean) => void
  ...
  const [headerJsonError, setHeaderJsonError] = React.useState<string | null>(null)
  const [payloadJsonText, setPayloadJsonText] = React.useState("")
  const [payloadJsonError, setPayloadJsonError] = React.useState<string | null>(null)
  ...
  const buildHeaderObject = React.useCallback(() => {
    ...
    return header
  }, [state.headerAlg, state.headerTyp, state.headerKid, state.headerExtraJson])
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      lastTokenRef.current = state.tokenText
      upsertInputEntry({ tokenText: state.tokenText }, paramsRef.current, "left", state.tokenText.slice(0, 80))
  ...
  }, [hasUrlParams, state.tokenText, paramsSnapshot, upsertInputEntry, upsertParams])
  ...
  const handleHeaderJsonChange = React.useCallback(
    (value: string) => {
      ...
      try {
        ...
        if (typeof headerObj.alg === "string" && algOptions.includes(headerObj.alg as JwtAlg)) {
        ...
        const extras = Object.fromEntries(
          Object.entries(headerObj).filter(([key]) => !["alg", "typ", "kid"].includes(key)),
        )
        setParam("headerExtraJson", Object.keys(extras).length ? JSON.stringify(extras, null, 2) : "")
      } catch (error) {
        setHeaderJsonError(error instanceof Error ? error.message : "Invalid JSON.")
      }
    },
    [setParam],
  )
  ...
  const handlePayloadJsonChange = React.useCallback(
    (value: string) => {
      ...
      try {
        ...
        setParam("iss", typeof payloadObj.iss === "string" ? payloadObj.iss : "")
        setParam("sub", typeof payloadObj.sub === "string" ? payloadObj.sub : "")
        setParam("jti", typeof payloadObj.jti === "string" ? payloadObj.jti : "")

        const expValue =
          typeof payloadObj.exp === "number" || typeof payloadObj.exp === "string" ? String(payloadObj.exp) : ""
        const nbfValue =
          typeof payloadObj.nbf === "number" || typeof payloadObj.nbf === "string" ? String(payloadObj.nbf) : ""
        const iatValue =
          typeof payloadObj.iat === "number" || typeof payloadObj.iat === "string" ? String(payloadObj.iat) : ""
        ...
        if (Array.isArray(payloadObj.aud)) {
          ...
          setParam("audListText", toTagListValue(payloadObj.aud.map((item) => String(item))))
          setParam("audText", "")
        ...
        const extras = Object.fromEntries(
          Object.entries(payloadObj).filter(([key]) => !registeredClaims.includes(key as RegisteredClaim)),
        )
        setParam("extraJsonText", Object.keys(extras).length ? JSON.stringify(extras, null, 2) : "")
      } catch (error) {
        setPayloadJsonError(error instanceof Error ? error.message : "Invalid JSON.")
      }
    },
    [setParam],
  )
  ...
  const secretWarning =
    isHmacAlg && state.secretText && state.secretText.length < 8 ? "Secret is short; verify carefully." : null
  ...
  const signatureTone = signatureStatus
    ? signatureStatus.status === "Valid"
      ...
      : "text-muted-foreground"
    : "text-muted-foreground"
  const labelCellClass = "py-1 pr-3 align-middle text-xs text-muted-foreground whitespace-nowrap w-20"
  ...
  const generateKeyMaterial = React.useCallback(async () => {
    ...
    try {
      ...
      const toPem = (buffer: ArrayBuffer, label: "PUBLIC KEY" | "PRIVATE KEY") => {
      ...
      setGenerateErrors([error instanceof Error ? error.message : "Key generation failed."])
    } finally {
      ...
      <div className="w-full min-w-0 space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            {copyHint && <p className="mt-1 text-xs text-muted-foreground">{copyHint}</p>}
          </div>
        ...
        {tokenWarning && <p className="text-xs text-muted-foreground">{tokenWarning}</p>}
        <div className="border-t pt-3 space-y-2">
                  ...
                  <td className="py-1">
                    <div className="flex items-center gap-2 min-w-0">
                      <Input
                        ...
                        onChange={(event) => setParam("secretText", event.target.value)}
                        placeholder="Enter secret"
                        className="h-8 min-w-0 flex-1 text-sm"
                      />
                      <Select
                        value={state.secretEncoding}
                        onValueChange={(value) => setParam("secretEncoding", value as JwtState["secretEncoding"], true)}
                      >
                ...
                <tr>
                  ...
                  <td className="py-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("secretPublicText", event.target.value)}
                ...
                <tr>
                  ...
                  <td className="py-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("secretPrivateText", event.target.value)}
          ...
          {secretWarning && <p className="text-xs text-muted-foreground">{secretWarning}</p>}
        ...
        <div className="border-t pt-3 space-y-2 text-sm">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <span className={cn("text-right", signatureTone)}>
              {signatureStatus ? `${signatureStatus.status} - ${signatureStatus.message}` : "—"}
            </span>
          </div>
        </div>
      </div>
          ...
          {state.headerView === "table" ? (
              ...
              <tbody>
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("alg", "Signing algorithm used for the token.")}
                  </td>
                  <td className="py-1">
                    <div className="flex items-center gap-3">
                      <Select
                        value={state.headerAlg}
                        onValueChange={(value) => setParam("headerAlg", value, true)}
                      >
                      ...
                      <div className="hidden sm:flex items-center gap-2">
                        ...
                        <Input value={state.headerTyp} readOnly className="h-8 w-28 bg-muted/50 text-sm" />
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("kid", "Key identifier used to select the signing key.")}
                  </td>
                  <td className="py-1">
                    <Input
                      value={state.headerKid}
                      onChange={(event) => setParam("headerKid", event.target.value)}
                ...
                <tr>
                  ...
                  <td className="py-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("headerExtraJson", event.target.value)}
                      placeholder='{"cty":"JWT"}'
                      rows={2}
                    ...
                    />
                    {headerExtraError && <p className="mt-1 text-xs text-destructive">{headerExtraError}</p>}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div className="space-y-2">
              <Textarea
                ...
                className={cn("min-h-[120px] break-all font-mono text-xs", headerJsonError && "border-destructive")}
                placeholder='{"alg":"HS256","typ":"JWT"}'
              />
              {headerJsonError && <p className="text-xs text-destructive">{headerJsonError}</p>}
            </div>
          )}
        </div>
          ...
          {state.payloadView === "table" ? (
            <table className="w-full">
              <tbody>
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("iss", "Issuer claim identifies who issued the JWT.")}
                  </td>
                  <td className="py-1">
                    <Input value={state.iss} onChange={(e) => setParam("iss", e.target.value)} className="h-8" />
                  </td>
                </tr>
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("sub", "Subject claim identifies the principal in the token.")}
                  </td>
                  <td className="py-1">
                    <Input value={state.sub} onChange={(e) => setParam("sub", e.target.value)} className="h-8" />
                  </td>
                </tr>
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("aud", "Audience claim identifies intended recipients.")}
                  </td>
                  <td className="py-1 space-y-2">
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Checkbox
                        checked={state.audIsArray}
                        onCheckedChange={(checked) => setParam("audIsArray", checked === true, true)}
                      />
                ...
                {(["expText", "iatText", "nbfText"] as const).map((key) => (
                  <tr key={key}>
                    <td className={labelCellClass}>
                      {key === "expText"
                        ? labelWithHelp("exp", "Expiration time in Unix seconds.")
                        : key === "iatText"
                        ? labelWithHelp("iat", "Issued at time in Unix seconds.")
                        : labelWithHelp("nbf", "Not before time in Unix seconds.")}
                    </td>
                    <td className="py-1">
                      <div className="flex items-center gap-2 min-w-0">
                        <Input
                ...
                <tr>
                  <td className={labelCellClass}>
                    {labelWithHelp("jti", "JWT ID provides a unique identifier for this token.")}
                  </td>
                  <td className="py-1">
                    <Input value={state.jti} onChange={(e) => setParam("jti", e.target.value)} className="h-8" />
                  </td>
                </tr>
                <tr>
                  <td className={labelCellClass}>Extra</td>
                  <td className="py-1">
                    <Textarea
                      ...
                      onChange={(event) => setParam("extraJsonText", event.target.value)}
                    ...
                    />
                    {extraError && <p className="mt-1 text-xs text-destructive">{extraError}</p>}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <div className="space-y-2">
              <Textarea
                ...
                onChange={(event) => handlePayloadJsonChange(event.target.value)}
                className={cn("min-h-[160px] break-all font-mono text-xs", payloadJsonError && "border-destructive")}
                placeholder='{"iss":"issuer","sub":"subject"}'
              />
              {payloadJsonError && <p className="text-xs text-destructive">{payloadJsonError}</p>}
            </div>
          )}
        </div>
