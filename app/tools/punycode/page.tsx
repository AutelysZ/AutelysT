...
import { toASCII, toUnicode, encodePunycode, decodePunycode } from "@/lib/encoding/punycode"
...
function PunycodeContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } = useUrlSyncedState("punycode", {
    schema: paramsSchema,
  ...
  const encodeText = React.useCallback(
    (text: string) => {
      try {
        ...
        const encoded = state.mode === "domain" ? toASCII(text) : encodePunycode(text)
  ...
  const decodeText = React.useCallback(
    (text: string) => {
      try {
        ...
        const decoded = state.mode === "domain" ? toUnicode(text) : decodePunycode(text)
...
function PunycodeInner({
  ...
  }, [state.leftText, state.rightText, state.activeSide, state.mode, upsertInputEntry])
  ...
  React.useEffect(() => {
    if (hasUrlParams && !hasHandledUrlRef.current) {
      ...
      if (activeText) {
        ...
        upsertParams({ mode: state.mode, activeSide: state.activeSide }, "interpretation")
      }
    }
  }, [hasUrlParams, state.leftText, state.rightText, state.activeSide, state.mode, upsertInputEntry, upsertParams])
  ...
  React.useEffect(() => {
    ...
    if (paramsRef.current.mode === nextParams.mode && paramsRef.current.activeSide === nextParams.activeSide) {
      return
    }
    paramsRef.current = nextParams
  ...
  return (
    <DualPaneLayout
      ...
      leftPlaceholder={state.mode === "domain" ? "Enter domain (e.g., example.com)" : "Enter text to encode..."}
      rightPlaceholder={state.mode === "domain" ? "Enter ASCII domain (e.g., xn--...)" : "Enter Punycode to decode..."}
    >
      <Card>
        <CardContent className="flex flex-wrap items-center gap-6 p-4">
          <div className="flex items-center gap-4">
            ...
            <RadioGroup
              value={state.mode}
              onValueChange={(v) => setParam("mode", v as "domain" | "label", true)}
