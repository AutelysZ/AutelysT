...
function CsvToolContent({
  ...
  React.useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      const activeFileId =
        state.activeFileId &&
        files.some((file) => file.id === state.activeFileId)
          ? state.activeFileId
          : files[0]?.id || "";
      saveCsvState({
        files,
  ...
  React.useEffect(() => {
    ...
    loadCsvState()
      .then((saved) => {
        ...
        const candidateId =
          state.activeFileId &&
          saved.files.some((file) => file.id === state.activeFileId)
            ? state.activeFileId
            : saved.activeFileId;
        const nextActiveId =
            ...
            if (existing) {
              const overwrite =
                typeof window !== "undefined"
                  ? window.confirm(
                      `File "${entry.name}" already exists. Overwrite?`,
                    )
                  : false;
  ...
  const activeRows = activeFile
    ? dataStoreRef.current.get(activeFile.id) || []
    : [];
  ...
  const handleAddRow = () => {
    ...
    const newRow = Array.from(
      { length: activeFile.columnCount || 1 },
      () => "",
    );
    rows.push(newRow);
    dataStoreRef.current.set(activeFile.id, rows);
        ...
        file.id === activeFile.id
          ? {
              ...
              columnWidthOverrides: Object.fromEntries(
                Object.entries(file.columnWidthOverrides).map(
                  ([key, value]) => {
                    const idx = Number(key);
                    return [idx >= safeIndex ? idx + count : idx, value];
                  },
  ...
  const handleFreezeChange = (nextRows: number, nextCols: number) => {
    ...
    setFiles((prev) =>
      prev.map((file) =>
        file.id === activeFile.id
          ? { ...file, freezeRows: clampedRows, freezeCols: clampedCols }
          : file,
      ),
    );
  ...
  const updateDefaultSizing = (
    key: "columnWidth" | "rowHeight",
    value: number,
  ) => {
    if (!activeFile) return;
    const clamped =
      key === "columnWidth"
        ? Math.max(40, Math.min(value, 800))
        : Math.max(20, Math.min(value, 200));
    setFiles((prev) =>
      prev.map((file) =>
        file.id === activeFile.id
          ? {
  ...
  const scanForMax = React.useCallback(
    async (rows: string[][], mode: "width" | "height") => {
  ...
  const handleFinishRename = () => {
    ...
    const conflict = files.some(
      (file) => file.name === finalName && file.id !== editingFileId,
    );
    if (conflict) {
  ...
  const handleRemoveFile = (id: string) => {
    ...
    setFiles(nextFiles);
    const nextActive =
      state.activeFileId === id ? nextFiles[0]?.id || "" : state.activeFileId;
  ...
  const inputWarning = oversizeKeys.length
    ? "Some parameters are not synced due to size limits."
    : null;
