...
export function CsvGrid({
  ...
  const extraHeight =
    rowOverrideEntries.prefix[rowOverrideEntries.prefix.length - 1] ?? 0;
  const totalHeight =
    headerHeight + Math.max(rowCount, 1) * defaultRowHeight + extraHeight;
  const visibleHeight = Math.max(0, viewportHeight - headerHeight);

  const adjustedScrollTop = Math.max(
    0,
    scrollTop - headerHeight - freezeRows * defaultRowHeight,
  );
    ...
    (event: PointerEvent) => {
      ...
      if (drag.type === "column") {
        const nextWidth = Math.max(
          40,
          Math.min(800, drag.startWidth + (event.clientX - drag.startX)),
        );
      ...
      if (drag.type === "row") {
        const nextHeight = Math.max(
          20,
          Math.min(200, drag.startHeight + (event.clientY - drag.startY)),
        );
      ...
      if (drag.type === "freezeCols") {
        const offset =
          event.clientX - drag.originLeft - ROW_NUMBER_WIDTH + scrollLeft;
      ...
      if (drag.type === "freezeRows") {
        const offset =
          event.clientY - drag.originTop - headerHeight + scrollTop;
        const nextRows = Math.max(
          0,
          Math.min(rowCount, Math.round(offset / defaultRowHeight)),
        );
  ...
  const freezeLineLeft =
    ROW_NUMBER_WIDTH + columnOffsets[freezeCols] + scrollLeft;
  const freezeLineTop =
    headerHeight +
    freezeRows * defaultRowHeight +
    getExtraBefore(freezeRows) +
    scrollTop;
  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent>
        ...
        <ContextMenuItem onSelect={onAddColumn}>
          Add column at end
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem
          ...
          <ContextMenuSubContent>
            {INSERT_COUNT_OPTIONS.map((count) => (
              <ContextMenuItem
                ...
                onSelect={() =>
                  handleInsertRows(Math.max(0, targetRow + 1), count)
                }
              >
                Add {count} row{count === 1 ? "" : "s"}
              </ContextMenuItem>
          ...
          <ContextMenuSubContent>
            {INSERT_COUNT_OPTIONS.map((count) => (
              <ContextMenuItem
                ...
                onSelect={() =>
                  handleInsertColumns(Math.max(0, targetCol), count)
                }
              >
          ...
          <ContextMenuSubContent>
            {INSERT_COUNT_OPTIONS.map((count) => (
              <ContextMenuItem
                ...
                onSelect={() =>
                  handleInsertColumns(Math.max(0, targetCol + 1), count)
                }
              >
                Add {count} column{count === 1 ? "" : "s"}
              </ContextMenuItem>
        ...
        <ContextMenuItem onSelect={onAutoAllColumns}>
          Auto-fit all columns
        </ContextMenuItem>
        <ContextMenuItem onSelect={onAutoAllRows}>
          Auto-fit all rows
        </ContextMenuItem>
        <ContextMenuSeparator />
        <ContextMenuItem
        ...
        <ContextMenuItem onSelect={() => onFreezeChange(0, 0)}>
          Unfreeze all
        </ContextMenuItem>
      </ContextMenuContent>
    </ContextMenu>
