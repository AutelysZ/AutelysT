...
function FormatSelector({
  ...
  const compressionFormats = COMPRESSION_FORMATS.filter(
    (f) => f.supportsCompression,
  );
...
function ArchiverInner() {
  ...
  // Sync mode with URL
  const mode: "compress" | "decompress" =
    searchParams.get("mode") === "decompress" ? "decompress" : "compress";
  const setMode = useCallback(
    (newMode: "compress" | "decompress") => {
  ...
  const [detectedFormat, setDetectedFormat] = React.useState<string | null>(
    null,
  );
  ...
  const decompressionFormats = COMPRESSION_FORMATS.filter(
    (f) => f.supportsDecompression,
  );
    ...
    try {
      for (const file of Array.from(uploadedFiles)) {
        ...
        try {
          ...
          console.log(
            `Decompressed ${file.name}: ${result.files.length} files, format: ${result.format}`,
          );
          ...
          if (
            ...
            throw new Error(
              `Failed to extract ${file.name}: ${err instanceof Error ? err.message : "Unknown error"}`,
            );
          }
        }
      }
      ...
      console.log(
        `Total extracted files: ${allExtractedFiles.length}`,
        allExtractedFiles,
      );
      ...
      setError(
        err instanceof Error ? err.message : "Failed to decompress files",
      );
    } finally {
  ...
  const prefixFilePaths = (nodes: FileNode[], prefix: string): FileNode[] => {
    return nodes.map((node) => ({
      ...
      children: node.children
        ? prefixFilePaths(node.children, prefix)
        : undefined,
    }));
  };
  ...
  const handlePasswordSubmit = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to decompress file",
      );
    } finally {
      setIsProcessing(false);
  ...
  const handleDownloadAll = async () => {
    ...
    try {
      ...
      const downloadFormatObj = COMPRESSION_FORMATS.find(
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <Tabs
        value={mode}
        onValueChange={(v) => setMode(v as "compress" | "decompress")}
      >
        <TabsList>
        ...
        <TabsContent value="compress" className="mt-4 space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-2">
              <Label className="text-sm">
                Password{" "}
                {!selectedFormat?.supportsPassword && "(not supported)"}
              </Label>
          ...
          {selectedFormat &&
            !selectedFormat.supportsMultipleFiles &&
            compressSelectedFiles > 1 && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  {selectedFormat.name} format only supports single file
                  compression. Only the first file will be compressed.
                </AlertDescription>
              </Alert>
            )}
          ...
          {extractedFiles.length > 0 ? (
            <Card>
              <CardHeader className="py-2 px-4">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-medium">
                    Extracted Files ({extractSelectedFiles}/{extractTotalFiles}{" "}
                    selected)
                  </CardTitle>
                  <span className="text-xs text-muted-foreground">
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {newFolderPath &&
                ` It will be created inside "${newFolderPath}".`}
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showPasswordDialog} onOpenChange={setShowPasswordDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This archive is password protected. Please enter the password to
              extract.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
