...
export function FileTree({
    ...
    const updateSelection = (
      ...
      return nodes.map((node) => {
        ...
        if (node.children) {
          return {
            ...node,
            children: updateSelection(node.children, parentSelected),
          };
        }
        return node;
      });
    ...
    const moveNode = (nodes: FileNode[]): FileNode[] => {
      ...
      return filtered.map((node) => {
        if (node.id === targetNode.id && node.children) {
          ...
          return {
            ...node,
            children: [...node.children, { ...draggedNode, path: newPath }],
          };
        }
        if (node.children) {
          return { ...node, children: moveNode(node.children) };
  ...
  return (
    <div className="text-sm">{nodes.map((node) => renderNode(node, 0))}</div>
  );
}
...
export function toggleAllSelection(
  ...
  return nodes.map((node) => ({
    ...
    children: node.children
      ? toggleAllSelection(node.children, selected)
      : undefined,
  }));
}
