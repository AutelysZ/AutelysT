...
import type { RawSourceMap, SourceFile, SourceMapBundle, SourceTreeNode } from "./source-map-viewer-types"
...
function buildSourceFiles(rawMap: RawSourceMap): SourceFile[] {
  ...
  return consumer.sources.map((source) => {
    const resolvedPath = normalizeSourcePath(joinSourceRoot(sourceRoot, source))
...
export function parseSourceMapText(name: string, content: string): { bundle: SourceMapBundle | null; error: string | null } {
  try {
    ...
    return {
      bundle: null,
      error: error instanceof Error ? error.message : "Failed to parse source map.",
    }
  }
}

export async function parseSourceMapsFromFiles(files: File[]): Promise<{ bundles: SourceMapBundle[]; errors: string[] }> {
  ...
  return bundles.map((bundle) => {
    ...
    for (const file of bundle.sources) {
      ...
      segments.forEach((segment, index) => {
        ...
        cursor.children = cursor.children ?? []
        let next = cursor.children.find((child) => child.type === "directory" && child.name === segment)
        if (!next) {
          next = {
...
function sanitizeZipPath(path: string): string {
  let normalized = normalizeSourcePath(path).replace(/:\/+/, "/").replace(/^[\/]+/, "")
...
export async function downloadSourceMapZip(bundles: SourceMapBundle[], zipName: string) {
  ...
  if (addedCount === 0) {
    return { ok: false as const, message: "No sources with embedded content to zip." }
  }
