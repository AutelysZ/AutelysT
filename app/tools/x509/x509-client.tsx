...
const paramsSchema = z.object({
  tab: z
    .enum(["create", "sign", "view", "validate", "convert"])
    .default("create"),
  ...
  createKeyType: z
    .enum(["rsa", "ec", "ed25519", "ed448", "dsa", "dh"])
    .default("rsa"),
  createEcCurve: z
    .enum([
      "prime256v1",
...
function X509Inner({ initialTab }: { initialTab?: string }) {
  const defaults = React.useMemo(
    () => paramsSchema.parse({ tab: initialTab }),
    [initialTab],
  );
  ...
  const historyLabel = React.useMemo(() => {
    if (state.tab === "create")
      return state.createSubjectDn.trim() || "X.509 create";
    if (state.tab === "sign")
      return state.signCsrInput.trim().slice(0, 80) || "X.509 sign";
    if (state.tab === "view")
      return state.viewInput.trim().slice(0, 80) || "X.509 view";
    if (state.tab === "validate")
      return state.validateInput.trim().slice(0, 80) || "X.509 validate";
  ...
  React.useEffect(() => {
    ...
    const shouldApplyHistoryValue = (key: string) => {
      ...
      const typedKey = key as keyof z.infer<typeof paramsSchema>;
      if (params.has(key) && !URL_EXCLUDED_KEYS.includes(typedKey))
        return false;
      return true;
    };
  ...
  React.useEffect(() => {
    ...
    if (
      snapshot === historySnapshotRef.current &&
      entries[0]?.hasInput === false
    )
      return;
  ...
  const [createOutput, setCreateOutput] = React.useState<CreateOutput | null>(
    null,
  );
  ...
  const [viewPrivateKeyDer, setViewPrivateKeyDer] =
  ...
  const [validateFileName, setValidateFileName] = React.useState<string | null>(
    null,
  );
  const [convertFileName, setConvertFileName] = React.useState<string | null>(
    null,
  );
  ...
  React.useEffect(() => {
    if (
      (state.createKeyType === "dsa" || state.createKeyType === "dh") &&
      state.createKeyMode === "generate"
    ) {
  ...
  const parsePkcs12Input = async (input: string, password?: string) => {
    try {
      ...
      const authenticatedSafe = pfx.parsedValue?.authenticatedSafe as
        | AuthenticatedSafe
        | undefined;
        ...
        for (const bag of safeContents.safeBags) {
          if (bag.bagId === PKCS12_BAG_IDS.certBag) {
            ...
            if (certBag.certValue instanceof asn1js.OctetString) {
              ...
              certBag.certValue &&
              typeof (certBag.certValue as any).toSchema === "function"
            ) {
      ...
      if (message.includes("contentEncryptionAlgorithm")) {
        ...
        const certs = parsed.certs.map(
          (cert) => new X509Certificate(certificateToDerBase64(cert)),
        );
        let privateKey: ArrayBuffer | undefined;
        if (parsed.privateKey) {
          const forgeModule = await import("node-forge");
          const forge = forgeModule.default ?? forgeModule;
          privateKey = base64ToArrayBuffer(
            forge.pki.privateKeyToPem(parsed.privateKey),
          );
        }
        return { certs, privateKey };
      }
  ...
  const parsePrivateKeyPkcs8 = (input: string) => {
    ...
    for (const curve of EC_CURVE_IDS) {
      try {
        ...
        return base64ToArrayBuffer(
          createEcPrivateKeyPem(curve, privateKey, publicKey),
        );
      } catch {}
    }
    ...
    throw new Error(
      "Unsupported private key format. Use PEM, DER (Base64), or JWK.",
    );
  };

  const jsbnToBigInt = (value: { toString: (radix: number) => string }) =>
  ...
  const buildJwkFromPublicKey = (key: CertPublicKey) => {
    ...
    const dhKey = key as {
      type: "dh";
      params: { p: bigint; g: bigint; y: bigint };
    };
    return {
      kty: "DH",
  ...
  const buildPublicKeyOutputs = (pem: string): ViewKeyOutput => {
    const outputs: ViewKeyOutput = {
      pem,
      derBase64: normalizeBase64Input(pem),
    };
    try {
  ...
  const parseCsrInput = (input: string, format: "pem" | "der") => {
    ...
    const subject =
      typeof params.subject?.str === "string" ? params.subject.str : "";
    const publicKeyPem =
      typeof params.sbjpubkey === "string" ? params.sbjpubkey : "";
    const publicKey = publicKeyPem ? parsePublicKeyPem(publicKeyPem) : null;
    const signatureAlgorithm =
      typeof params.sigalg === "string" ? params.sigalg : "";
    const extensions = params.extreq
      ? JSON.stringify(params.extreq, null, 2)
      : "[]";
    const fingerprint = bytesToHex(
      sha256(new Uint8Array(base64ToArrayBuffer(derBase64))),
    );
    return {
      pem,
      derBase64,
  ...
  const getPublicKeyPemFromCert = async (cert: X509Certificate) => {
    ...
    try {
      const spki = await globalThis.crypto.subtle.exportKey(
        "spki",
        cert.publicKey as unknown as CryptoKey,
      );
  ...
  const buildCsrSummary = (
    csr: ReturnType<typeof parseCsrInput>,
  ): ViewSummary => ({
    kind: "csr",
    subject: csr.subject,
  ...
  const createPkcs12Base64Pkijs = async (
    ...
    const safeContents = new SafeContents({
      safeBags: [
        new SafeBag({
          bagId: PKCS12_BAG_IDS.pkcs8ShroudedKeyBag,
          bagValue: keyBag,
        }),
  ...
  const buildX509Summary = (cert: X509Certificate): ViewSummary => {
    ...
    const publicKeyAlgorithm =
      (cert.publicKey?.algorithm as { name?: string } | undefined)?.name ?? "";
  ...
  const handleViewUpload = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const result = await readFileAsInput(
        file,
  ...
  const handleSignUpload = React.useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const result = await readFileAsInput(
        file,
        state.signCsrFormat as X509InputFormat,
      );
      const format = result.format === "pkcs12" ? "pem" : result.format;
      setParam(
        "signCsrFormat",
        format as z.infer<typeof paramsSchema>["signCsrFormat"],
        true,
      );
  ...
  const parseSigningKey = (input: string): SigningKey => {
    ...
    try {
      ...
      if (
        ...
        err.message.includes("DH keys cannot sign certificates.")
      ) {
        throw err;
      }
    }
    throw new Error(
      "Unsupported issuer key format. Use PEM, DER (Base64), or JWK.",
    );
  };
  ...
  const handleCreate = React.useCallback(async () => {
    ...
    try {
      ...
      const bytesToBinaryString = (value: Uint8Array) =>
        forge.util.createBuffer(value).getBytes();
        ...
        if (state.createKeyType === "rsa") {
          const keyPair = await new Promise<any>((resolve, reject) => {
            forge.pki.rsa.generateKeyPair(
              { bits: state.createKeySize, workers: 2 },
              (err: Error, pair: any) => {
                if (err) reject(err);
                else resolve(pair);
              },
          ...
          subjectPrivateKeyPem = createEcPrivateKeyPem(
            state.createEcCurve,
            privateKey,
            publicKey,
          );
        } else if (
          state.createKeyType === "ed25519" ||
          state.createKeyType === "ed448"
        ) {
          const curve = state.createKeyType === "ed25519" ? "Ed25519" : "Ed448";
          const { privateKey, publicKey } = generateEdKeyPair(curve);
          subjectPublicKey = {
            type: state.createKeyType,
            publicKeyBytes: publicKey,
          };
          ...
          throw new Error(
            "DSA key generation is not supported. Provide a DSA private key.",
          );
        } else {
          throw new Error(
            "DH key generation is not supported. Provide a DH private key.",
          );
        }
      } else {
        if (!state.createPrivateKeyPem.trim()) {
          throw new Error(
            "Private key input is required when using an existing key.",
          );
        }
        if (state.createKeyType === "rsa") {
          ...
          const { privateKey, publicKey } = parseEcPrivateKey(
            state.createPrivateKeyPem,
            state.createEcCurve,
          );
          if (!publicKey) {
          ...
          const curve = state.createKeyType === "ed25519" ? "Ed25519" : "Ed448";
          const { privateKey, publicKey } = parseEdPrivateKey(
            state.createPrivateKeyPem,
            curve,
          );
          subjectPublicKey = {
      ...
      if (state.createOutputType === "csr") {
        ...
        if (isEdKey) {
          throw new Error(
            "Ed25519 and Ed448 certificate requests are not supported.",
          );
        }
        ...
        const buildCsrPublicKey = () => {
          ...
          if (subjectPublicKey?.type === "ec") {
            ...
            try {
              ...
              throw new Error(
                "Selected EC curve is not supported for CSR generation.",
              );
            }
          }
        ...
        const buildCsrExtensions = () => {
          ...
          if (state.createSubjectKeyIdentifier) {
            const keyIdentifier = computeSubjectKeyIdentifier(
              publicKeyToAsn1(subjectPublicKey as CertPublicKey),
            );
        ...
        const csrDerBase64 = state.createIncludeDer
          ? hextob64(csr.tohex())
          : undefined;
      ...
      if (!state.createSelfSigned) {
        if (
          !state.createIssuerCertPem.trim() ||
          !state.createIssuerKeyPem.trim()
        ) {
          throw new Error(
            "Issuer certificate and issuer key are required for non-self-signed certificates.",
          );
        }
      ...
      const subjectKeyIdentifier = computeSubjectKeyIdentifier(
        publicKeyToAsn1(subjectPublicKey),
      );
      const originalGenerateSubjectKeyIdentifier =
        cert.generateSubjectKeyIdentifier;
      cert.generateSubjectKeyIdentifier = () =>
        forge.util.createBuffer(subjectKeyIdentifier);
      ...
      try {
        ...
        if (state.createAuthorityKeyIdentifier) {
          ...
          if (issuerCert?.publicKey) {
            try {
              const issuerSpki = forge.pki.publicKeyToAsn1(
                issuerCert.publicKey,
              );
              keyIdentifier = computeSubjectKeyIdentifier(issuerSpki);
            } catch {
              keyIdentifier =
                issuerCert?.generateSubjectKeyIdentifier?.().getBytes?.() ??
                true;
            }
          }
        ...
        if (signingKey.kind === "rsa") {
          const signatureHex = signWithJsrsasign(
            RSA_SIGNATURE_ALGOS[hashAlgorithm],
            signingKey.key,
            tbsBytes,
          );
          ...
          const signatureHex = signWithJsrsasign(
            DSA_SIGNATURE_ALGOS[hashAlgorithm],
            signingKey.key,
            tbsBytes,
          );
          signature = forge.util.hexToBytes(signatureHex);
          ...
          const digest =
            hashAlgorithm === "sha256"
              ...
              : hashAlgorithm === "sha384"
                ? sha384(tbsBinary)
                : sha512(tbsBinary);
          const sigBytes = spec.noble.sign(digest, signingKey.privateKey, {
            prehash: false,
            format: "der",
          });
        ...
        const derBase64 = state.createIncludeDer
          ? certificateToDerBase64(cert)
          : undefined;
        const pkcs12Base64 = state.createIncludePkcs12
          ? state.createKeyType === "rsa"
            ? createPkcs12Base64(
                cert,
                forge.pki.privateKeyFromPem(subjectPrivateKeyPem),
                state.createPkcs12Password,
              )
            : (() => {
        ...
        cert.generateSubjectKeyIdentifier =
          originalGenerateSubjectKeyIdentifier;
      }
    } catch (err) {
      console.error(err);
      setCreateError(
  ...
  }, [state, notBeforeDate, notAfterDate]);
  ...
  const handleSign = React.useCallback(async () => {
    ...
    try {
      ...
      cert.validity.notAfter =
        signNotAfterDate ?? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
      cert.setSubject(subjectAttrs);
      cert.setIssuer(issuerAttrs);
      ...
      try {
        ...
        const certPem = certificateToPem(cert);
        const derBase64 = state.signIncludeDer
          ? certificateToDerBase64(cert)
          : undefined;
  ...
  React.useEffect(() => {
    ...
    const parse = async () => {
      try {
        ...
        const looksLikeCsr =
          format !== "pkcs12" &&
          /BEGIN (NEW )?CERTIFICATE REQUEST/.test(trimmed);
  ...
  React.useEffect(() => {
    ...
    const update = async () => {
      try {
        const selected =
          viewSummaries[state.viewSelectedIndex] ?? viewSummaries[0];
        if (!selected) {
  ...
  const handleValidate = React.useCallback(async () => {
    ...
    try {
      ...
      if (state.validateCaBundle.trim()) {
        try {
          const caParsed = await parseX509Certificates(
            state.validateCaBundle,
            "pem",
          );
  ...
  const handleConvert = React.useCallback(async () => {
    ...
    try {
      ...
      if (state.convertOutputPkcs12) {
        ...
        output.pkcs12 = await createPkcs12Base64Pkijs(
          cert,
          privateKeyDer,
          state.convertP12Password,
        );
      ...
      console.error(err);
      setConvertError(
        err instanceof Error ? err.message : "Failed to convert certificate.",
      );
    } finally {
      setConvertBusy(false);
    }
  ...
  const handleCreateZip = async () => {
    ...
    if (state.createOutputType === "csr") {
      ...
      if (state.createIncludeDer && createOutput.csrDerBase64) {
        zip.file("request.der", atob(createOutput.csrDerBase64), {
          binary: true,
        });
      }
    } else {
      ...
      if (state.createIncludeDer && createOutput.derBase64) {
        zip.file("certificate.der", atob(createOutput.derBase64), {
          binary: true,
        });
      }
      if (state.createIncludePkcs12 && createOutput.pkcs12Base64) {
        zip.file("certificate.p12", atob(createOutput.pkcs12Base64), {
          binary: true,
        });
      }
    }
  ...
  const handleConvertZip = async () => {
    ...
    if (convertOutput.der)
      zip.file("certificate.der", atob(convertOutput.der), { binary: true });
    if (convertOutput.pkcs12)
      zip.file("certificate.p12", atob(convertOutput.pkcs12), { binary: true });
    const blob = await zip.generateAsync({ type: "blob" });
  ...
  const handleViewDownloadAllZip = async () => {
    ...
    if (viewPemOutputs.length > 0) {
      ...
      zip.file(
        `${certFolder}/${isCsrView ? "request" : "certificate"}.pem`,
        pem,
      );
      if (der) {
        zip.file(
          `${certFolder}/${isCsrView ? "request" : "certificate"}.der`,
          atob(der),
          { binary: true },
        );
      }
    ...
    if (viewKeyOutputs?.publicKey) {
      ...
      if (pk.derBase64)
        zip.file("public-key/public-key.der", atob(pk.derBase64), {
          binary: true,
        });
    ...
    if (viewKeyOutputs?.privateKey) {
      ...
      if (pk.derBase64)
        zip.file("private-key/private-key.der", atob(pk.derBase64), {
          binary: true,
        });
    ...
    downloadBlob(
      blob,
      isCsrView ? "x509-csr-bundle.zip" : "x509-certificate-bundle.zip",
    );
  };
  ...
  const viewDerOutputs = React.useMemo(() => {
    ...
    if (viewCerts.length)
      return viewCerts.map((cert) => cert.toString("base64"));
    return [];
  }, [viewCsrData, viewCerts]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <Tabs
        value={state.tab}
        onValueChange={(value) =>
          setParam("tab", value as z.infer<typeof paramsSchema>["tab"], true)
        }
      >
          ...
          <div className="grid gap-4">
            ...
            {isCsr && (
              <p className="text-xs text-muted-foreground">
                CSR output ignores issuer, validity, serial, and PKCS#12
                options.
              </p>
            )}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {isCsr && (
                  <p className="text-xs text-muted-foreground">
                    CSR output uses the subject and key only. Issuer inputs are
                    ignored.
                  </p>
                )}
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setParam("createSubjectDn", e.target.value)
                    }
                ...
                <div className="flex items-center justify-between rounded-md border px-3 py-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Use subject as issuer and sign with the same key.
                    </p>
                  </div>
                  <Switch
                    checked={state.createSelfSigned}
                    onCheckedChange={(checked) =>
                      setParam("createSelfSigned", checked, true)
                    }
                    disabled={isCsr}
                  />
                </div>
                  ...
                  <>
                    <div className="space-y-2">
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          setParam("createIssuerDn", e.target.value)
                        }
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setParam("createIssuerCertPem", e.target.value)
                          }
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <Label>
                            Issuer Private Key (PEM, DER Base64, or JWK)
                          </Label>
                          <UploadButton
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setParam("createIssuerKeyPem", e.target.value)
                          }
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  ...
                  <div className="space-y-2">
                    ...
                    {!allowGenerateKey && (
                      <p className="text-xs text-muted-foreground">
                        DSA and DH keys must be provided.
                      </p>
                    )}
                  </div>
                  <div className="space-y-2">
                ...
                {isEcKey && (
                  <div className="space-y-2">
                    ...
                    {state.createKeyMode === "provided" && (
                      <p className="text-xs text-muted-foreground">
                        Curve must match the provided key.
                      </p>
                    )}
                  </div>
                )}
                ...
                {state.createKeyMode === "provided" && (
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setParam("createPrivateKeyPem", e.target.value)
                      }
                      placeholder="-----BEGIN PRIVATE KEY----- or JWK JSON"
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setParam(
                          "createHash",
                          value as z.infer<typeof paramsSchema>["createHash"],
                          true,
                        )
                      }
                    ...
                    {state.createSelfSigned && isEdKey && (
                      <p className="text-xs text-muted-foreground">
                        Ed25519 and Ed448 signatures do not use a hash selector.
                      </p>
                    )}
                    {state.createSelfSigned && isDhKey && (
                      <p className="text-xs text-muted-foreground">
                        DH keys cannot sign certificates.
                      </p>
                    )}
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setParam(
                            "createBasicConstraintsCritical",
                            checked,
                            true,
                          )
                        }
                      ...
                      <span className="text-sm text-muted-foreground">
                        Mark basic constraints critical
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <DateTimePicker
                      ...
                      buttonLabel={
                        notBeforeDate ? notBeforeDate.toISOString() : "Pick"
                      }
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <DateTimePicker
                      ...
                      buttonLabel={
                        notAfterDate ? notAfterDate.toISOString() : "Pick"
                      }
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setParam("createIsCa", checked, true)
                    }
                  />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setParam("createSanCritical", checked, true)
                      }
                    />
                    <span className="text-xs text-muted-foreground">
                      Mark SAN critical
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2 rounded-md border p-3 text-sm">
                      {KEY_USAGE_OPTIONS.map((item) => {
                        const checked = state.createKeyUsage
                          .split(",")
                          .includes(item.key);
                        return (
                          <label
                      ...
                      <label className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            setParam("createKeyUsageCritical", checked, true)
                          }
                        />
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid gap-2 rounded-md border p-3 text-sm">
                      {EXT_KEY_USAGE_OPTIONS.map((item) => {
                        const checked = state.createExtKeyUsage
                          .split(",")
                          .includes(item.key);
                        return (
                          <label
                      ...
                      <label className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            setParam("createExtKeyUsageCritical", checked, true)
                          }
                        />
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("createIncludePem", Boolean(value), true)
                      }
                    />
                    {isCsr ? "CSR PEM" : "PEM"}
                  </label>
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("createIncludeDer", Boolean(value), true)
                      }
                    />
                  ...
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("createIncludePkcs12", Boolean(value), true)
                      }
                  ...
                  {state.createIncludePkcs12 && isRsaKey && !isCsr && (
                    <Input
                      ...
                      onChange={(e) =>
                        setParam("createPkcs12Password", e.target.value)
                      }
                      className="w-60"
                    />
                  )}
                  {!isCsr && !isRsaKey && (
                    <span className="text-xs text-muted-foreground">
                      PKCS#12 output is available for RSA keys only.
                    </span>
                  )}
                </div>
                ...
                {createOutput && (
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Textarea
                        value={createOutput.privateKeyPem}
                        readOnly
                        className="min-h-[160px] font-mono text-xs whitespace-pre-wrap break-all"
                      />
                      <Button
                    ...
                    {state.createIncludeDer &&
                      (isCsr
                        ? createOutput.csrDerBase64
                        : createOutput.derBase64) && (
                        <div className="space-y-2">
                          <Label>
                            {isCsr
                              ? "Certificate Request (DER Base64)"
                    ...
                    {!isCsr &&
                      state.createIncludePkcs12 &&
                      createOutput.pkcs12Base64 && (
                        <div className="space-y-2">
                          <Label>PKCS#12 (Base64)</Label>
                          <Textarea
                            value={createOutput.pkcs12Base64}
                            readOnly
                            className="min-h-[140px] font-mono text-xs whitespace-pre-wrap break-all"
                          />
                          <Button
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) =>
                          setParam("signUseCsrExtensions", checked, true)
                        }
                      />
                      <span className="text-xs text-muted-foreground">
                        Include requested extensions in the signed cert.
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setParam("signIssuerCertPem", e.target.value)
                      }
                      placeholder="-----BEGIN CERTIFICATE-----"
                      className="min-h-[140px] font-mono text-xs whitespace-pre-wrap break-all"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setParam("signIssuerKeyPem", e.target.value)
                      }
                      placeholder="-----BEGIN PRIVATE KEY----- or JWK JSON"
                      className="min-h-[140px] font-mono text-xs whitespace-pre-wrap break-all"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) =>
                        setParam(
                          "signHash",
                          value as z.infer<typeof paramsSchema>["signHash"],
                          true,
                        )
                      }
                    >
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("signIncludePem", Boolean(value), true)
                      }
                    />
                    PEM
                  </label>
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("signIncludeDer", Boolean(value), true)
                      }
                    />
                    DER (Base64)
                  </label>
                  ...
                  {signOutput && (
                    <>
                      ...
                      <Button
                        variant="outline"
                        onClick={() =>
                          handleCopy(signOutput.certPem ?? "", setSignCopied)
                        }
                      >
                        {signCopied ? (
                          <Check className="h-4 w-4" />
            ...
            <div className="grid gap-4 md:grid-cols-[1.1fr_1fr]">
                ...
                <CardContent className="space-y-3">
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          setParam(
                            "viewFormat",
                            value as z.infer<typeof paramsSchema>["viewFormat"],
                            true,
                          )
                        }
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setParam("viewPassword", e.target.value)
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Textarea
                      ...
                      onChange={(e) =>
                        setParam("viewPrivateKeyPem", e.target.value)
                      }
                      placeholder="-----BEGIN PRIVATE KEY----- or JWK JSON"
                      className="min-h-[100px] font-mono text-xs whitespace-pre-wrap break-all"
                    />
              ...
              <Card className="h-full">
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle>
                    {viewIsCsr ? "CSR Details" : "Certificate Details"}
                  </CardTitle>
                  {selectedViewSummary && (
                    <Button
                      variant="ghost"
                      ...
                      onClick={() =>
                        handleCopy(
                          selectedViewSummary.extensions,
                          setViewCopied,
                        )
                      }
                    >
                    ...
                    <>
                      ...
                      {selectedViewSummary && (
                        <>
                          ...
                          <InfoRow
                            label="Subject"
                            value={selectedViewSummary.subject}
                          />
                          {selectedViewSummary.kind === "certificate" && (
                            <>
                              <InfoRow
                                label="Issuer"
                                value={selectedViewSummary.issuer ?? ""}
                              />
                              <InfoRow
                                label="Serial"
                                value={selectedViewSummary.serial ?? ""}
                                mono
                              />
                              <InfoRow
                                label="Valid From"
                                value={selectedViewSummary.notBefore ?? ""}
                              />
                              <InfoRow
                                label="Valid To"
                                value={selectedViewSummary.notAfter ?? ""}
                              />
                              <InfoRow
                                label="Is CA"
                                value={selectedViewSummary.isCa ? "Yes" : "No"}
                              />
                            </>
                          )}
                          <InfoRow
                            ...
                            value={
                              selectedViewSummary.signatureAlgorithm || "N/A"
                            }
                          />
                          <InfoRow
                            label="Public Key Algorithm"
                            value={
                              selectedViewSummary.publicKeyAlgorithm || "N/A"
                            }
                          />
                          <InfoRow
                            label="SHA-256 Fingerprint"
                            value={selectedViewSummary.fingerprintSha256}
                            mono
                          />
                          <div className="space-y-2">
                            <Label>Extensions</Label>
                            <Textarea
                              value={selectedViewSummary.extensions}
                              readOnly
                              className="min-h-[120px] font-mono text-xs whitespace-pre-wrap break-all"
                            />
                          </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          <Button
                            ...
                            onClick={() =>
                              downloadTextFile(
                                viewPemOutputs[state.viewSelectedIndex] ??
                                  viewPemOutputs[0],
                                viewIsCsr ? "request.csr" : "certificate.pem",
                              )
                            }
                          >
                            <Download className="h-4 w-4" />
                          ...
                          <Button
                            ...
                            onClick={() =>
                              downloadBase64File(
                                viewDerOutputs[state.viewSelectedIndex] ??
                                  viewDerOutputs[0],
                                viewIsCsr ? "request.der" : "certificate.der",
                              )
                            }
                          >
                            <Download className="h-4 w-4" />
              ...
              <CardContent className="space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setParam("validatePassword", e.target.value)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setParam("validateCaBundle", e.target.value)
                    }
                    placeholder="-----BEGIN CERTIFICATE-----"
                    className="min-h-[120px] font-mono text-xs whitespace-pre-wrap break-all"
                  />
              ...
              <CardContent className="space-y-3">
                {validateResult ? (
                  <>
                    <StatusRow
                      label="Time Valid"
                      ok={validateResult.timeValid}
                    />
                    <StatusRow
                      label="Signature Valid"
                      ok={validateResult.signatureValid}
                    />
                    <StatusRow
                      label="Chain Valid"
                      ok={validateResult.chainValid}
                    />
              ...
              <CardContent className="space-y-3">
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setParam("convertPassword", e.target.value)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      setParam("convertPrivateKeyPem", e.target.value)
                    }
                    placeholder="-----BEGIN PRIVATE KEY----- or JWK JSON"
                    className="min-h-[120px] font-mono text-xs whitespace-pre-wrap break-all"
                  />
                ...
                <div className="flex flex-wrap gap-4 text-sm">
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("convertOutputPem", Boolean(value), true)
                      }
                    />
                    PEM
                  </label>
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("convertOutputDer", Boolean(value), true)
                      }
                    />
                    DER (Base64)
                  </label>
                  <label className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(value) =>
                        setParam("convertOutputPkcs12", Boolean(value), true)
                      }
                    />
                    PKCS#12
                  </label>
                  {state.convertOutputPkcs12 && (
                    <Input
                      ...
                      onChange={(e) =>
                        setParam("convertP12Password", e.target.value)
                      }
                      className="w-60"
                    />
                  )}
