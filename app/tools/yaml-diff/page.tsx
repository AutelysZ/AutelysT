...
import { Upload, Copy, Check, FileText, AlertCircle, Maximize2, Minimize2 } from "lucide-react"
...
function YAMLDiffContent() {
  ...
  return (
    <ToolPageWrapper
      ...
      description="Compare two YAML files or inputs and view differences"
...
function YAMLDiffInner({
  ...
  // Compute text diff for text view mode
  const { textHunks, textStats } = React.useMemo(() => {
    if (!leftParsed || !rightParsed) {
      return { textHunks: [], textStats: { added: 0, removed: 0, unchanged: 0 } }
    }
    const leftFormatted = yaml.dump(leftParsed, { indent: 2, lineWidth: -1 })
    const rightFormatted = yaml.dump(rightParsed, { indent: 2, lineWidth: -1 })
    const diff = computeTextDiff(leftFormatted, rightFormatted)
    const hunks = groupIntoHunks(diff, 3)
    const stats = getDiffStats(diff)
    return { textHunks: hunks, textStats: stats }
  ...
  const hasValidYaml = leftParsed !== null && rightParsed !== null
  ...
  return (
    <div className={cn("flex flex-col gap-4", isFullscreen && "fixed inset-0 z-50 bg-background p-4")}>
        ...
        <div className="flex gap-4">
          ...
          <div className="flex w-0 flex-1 flex-col gap-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">Original YAML {leftFileName && `(${leftFileName})`}</Label>
          ...
          <div className="flex w-0 flex-1 flex-col gap-2">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium">Modified YAML {rightFileName && `(${rightFileName})`}</Label>
              <div className="flex gap-1">
                <input
                  ref={rightInputRef}
                  type="file"
      ...
      {/* Diff Results */}
      {hasValidYaml && (changes.length > 0 || textHunks.length > 0) && (
      ...
      {hasValidYaml && changes.length === 0 && (
        <Alert>
          <FileText className="h-4 w-4" />
          <AlertDescription>The two YAML documents are identical.</AlertDescription>
        </Alert>
      )}
    </div>
  )
  ...
  return (
    <div className="border-b last:border-0">
      ...
      {hunk.lines.map((line, idx) => {
        ...
        return <TextDiffLineComponent key={idx} line={line} />
      })}
    </div>
  )
}

function TextDiffLineComponent({ line }: { line: TextDiffLine }) {
  const bgClass =
    line.type === "added"
