...
function AsymmetricEncryptionContent() {
  const {
    ...
    hydrationSource,
    resetToDefaults,
  } = useUrlSyncedState("asymmetric-encryption", {
    schema: paramsSchema,
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      (Object.keys(paramsSchema.shape) as (keyof AsymmetricState)[]).forEach(
        (key) => {
...
function AsymmetricEncryptionInner({
  ...
  setParam: <K extends keyof AsymmetricState>(
    key: K,
    value: AsymmetricState[K],
    immediate?: boolean,
  ) => void;
  ...
  const messageLimitWarning =
    ...
    messageByteLength > maxMessageBytes;
    ...
    const run = async () => {
      try {
        ...
        if (state.mode === "encrypt") {
          const messageBytes = decodeMessageBytes(
            state.input,
            state.messageEncoding,
          );
          ...
          const cipherBytes = decodeCipherBytes(
            state.input,
            state.cipherEncoding,
          );
        ...
        if (runRef.current !== runId) return;
        setError(
          err instanceof Error ? err.message : "Failed to process encryption.",
        );
        setOutput("");
      } finally {
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="flex flex-col gap-4 overflow-x-hidden">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <Textarea
                value={
                  state.mode === "encrypt" ? state.publicKey : state.privateKey
                }
                onChange={(event) =>
                  setParam(
                    state.mode === "encrypt" ? "publicKey" : "privateKey",
                    event.target.value,
                  )
                }
                placeholder={
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleKeyUploadClick(
                        state.mode === "encrypt" ? "public" : "private",
                      )
                    }
              ...
              {oversizeKeys.includes(
                ...
                <p className="text-xs text-muted-foreground">
                  {state.mode === "encrypt" ? "Public" : "Private"} key exceeds
                  2 KB and is not synced to the URL.
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <Label className="text-sm">
                {state.mode === "encrypt" ? "Message" : "Ciphertext"}
              </Label>
              <Tabs
                value={
                  state.mode === "encrypt"
            ...
            {state.mode === "encrypt" && (
              ...
              >
                Max message size: {maxMessageBytes} bytes for RSA-OAEP (
                {state.rsaHash}).
              </p>
            )}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              <Label className="text-sm">
                {state.mode === "encrypt" ? "Ciphertext" : "Message"}
              </Label>
              <Tabs
                value={
                  state.mode === "encrypt"
