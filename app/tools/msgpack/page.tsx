...
import {
  ...
  encodeMsgpack,
  decodeMsgpack,
  decodeMsgpackWithDetails,
  validateJsonForMsgpack,
  validateYamlForMsgpack,
  objectToJson,
  objectToYaml,
  type MsgpackField,
} from "@/lib/msgpack/codec"
...
export default function MsgpackCodecPage() {
  return (
...
function MsgpackCodecContent() {
  const { state, setParam, oversizeKeys, resetToDefaults } =
    useUrlSyncedState("msgpack", {
      schema: paramsSchema,
...
function MsgpackCodecInner({
  ...
  const [decodedFields, setDecodedFields] = React.useState<MsgpackField[]>([])
    ...
    void (async () => {
      try {
        ...
        if (state.mode === "decode") {
          ...
          const decoded = decodeMsgpack(inputData)
          const details = decodeMsgpackWithDetails(inputData)
          ...
          if (state.inputFormat === "json") {
            const validation = validateJsonForMsgpack(state.input)
            if (!validation.isValid) {
              throw new Error(`Invalid JSON: ${validation.error}`)
            }
            inputData = validation.parsed
          } else {
            const validation = validateYamlForMsgpack(state.input)
            if (!validation.isValid) {
          ...
          const encoded = encodeMsgpack(inputData)
        ...
        if (result.binary) {
          ...
          setBinaryMeta({ name: "output.msgpack", size: result.binary.length })
          setOutput("")
        } else {
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <Label className="text-sm font-medium">
              {state.mode === "decode" ? "MessagePack Data" : "Data to Encode"}
            </Label>
          ...
          <div className="relative">
            <Textarea
              ...
              placeholder={
                state.mode === "decode"
                  ? "Paste Base64/Hex encoded MessagePack data or upload a file..."
...
function renderFields(fields: MsgpackField[], depth = 0): React.ReactNode[] {
