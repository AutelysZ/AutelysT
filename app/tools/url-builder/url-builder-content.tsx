...
const defaultUrl = "https://example.com/path?ref=autelyst#section/overview?tab=details"

export default function UrlBuilderContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } = useUrlSyncedState("url-builder", {
    schema: paramsSchema,
  ...
  const normalizedEncoding = React.useMemo(() => normalizeEncoding(state.encoding), [state.encoding])
  const parsed = React.useMemo(() => parseUrl(state.url, normalizedEncoding), [state.url, normalizedEncoding])
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      if (params.encoding) setParam("encoding", normalizeEncoding(params.encoding as string), true)
    },
    [setParam],
  )
