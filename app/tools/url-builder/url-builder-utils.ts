...
const SORTED_ENCODINGS = ICONV_ENCODING_KEYS.filter((key) => key !== "utf8").sort((a, b) => a.localeCompare(b))
...
export function buildEncodedQuery(params: UrlParam[], encoding: string): string {
  const queryString = buildParams(params, encoding)
  return queryString ? `?${queryString}` : ""
}

export function buildEncodedHash(hashPathname: string, params: UrlParam[], encoding: string): string {
...
export function parseUrl(input: string, encoding: string): ParsedUrlData {
  ...
  try {
    ...
    const queryRaw = typeof parsed.query === "string" ? parsed.query.replace(/^\?/, "") : ""
...
export function buildUrl(data: ParsedUrlData, encoding: string): string {
  ...
  const hasAuthority = Boolean(protocol || hostname || port || username || password)
  ...
  const hashValue = hashPath || hashQuery ? `${hashPath}${hashQuery ? `?${hashQuery}` : ""}` : ""
