...
export default function UrlParamEditor({
  ...
  const handleParamChange = React.useCallback(
    (index: number, nextKey: string, nextValue: string) => {
      const updated = params.map((param, i) =>
        i === index ? { key: nextKey, value: nextValue } : param,
      );
      onChange(updated);
    },
    [onChange, params],
  ...
  const handleCopy = React.useCallback(
    async (param: UrlParam, index: number) => {
          ...
          {params.map((param, index) => (
            ...
            >
              <Input
                value={param.key}
                onChange={(event) =>
                  handleParamChange(index, event.target.value, param.value)
                }
                placeholder="Key"
              ...
              <div className="relative min-w-[160px] flex-1">
                <Input
                  value={param.value}
                  onChange={(event) =>
                    handleParamChange(index, param.key, event.target.value)
                  }
                  placeholder="Value"
                  className="pr-9"
                />
