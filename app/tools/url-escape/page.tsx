...
const modeOptions = [
  { value: "all", label: "Encode All", description: "Encode all bytes to %XX" },
  {
    value: "component",
    label: "Component",
    description: "Keep unreserved chars (A-Z, a-z, 0-9, -, ., _, ~)",
  },
  {
    value: "reserved",
    label: "Keep Reserved",
    description: "Keep unreserved + reserved chars (: / ? # etc.)",
  },
];

function UrlEscapeContent() {
  const { state, setParam, oversizeKeys, hasUrlParams, hydrationSource } =
    useUrlSyncedState("url-escape", {
      schema: paramsSchema,
  ...
  const encodeToUrlEscape = React.useCallback(
    (text: string) => {
      try {
        ...
        const encoded = encodeUrlEscape(bytes, {
          upperCase: state.upperCase,
          mode: state.mode as EncodeMode,
        });
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      if (params.mode)
        setParam("mode", params.mode as "all" | "component" | "reserved");
      ...
      <Card>
        <CardContent className="flex flex-wrap items-center gap-6 p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              value={state.mode}
              onValueChange={(v) => setParam("mode", v as EncodeMode, true)}
            >
