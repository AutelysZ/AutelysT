...
import type { BencodeField } from "@/lib/bencode/codec";
import {
  decodeInputData,
  encodeOutputData,
  encodeBencode,
  decodeBencode,
  decodeBencodeWithDetails,
  validateJsonForBencode,
  validateYamlForBencode,
  objectToJson,
  objectToYaml,
  type InputEncoding,
  type OutputEncoding,
...
import type { BencodeState } from "./bencode-types";

export default function BencodeInner({
  state,
  setParam,
  oversizeKeys,
  resetToDefaults,
  ...
  state: BencodeState;
  setParam: <K extends keyof BencodeState>(
    key: K,
    value: BencodeState[K],
    immediate?: boolean,
  ) => void;
  oversizeKeys: (keyof BencodeState)[];
  ...
  const [decodedFields, setDecodedFields] = React.useState<BencodeField[]>([]);
    ...
    void (async () => {
      try {
        ...
        if (state.mode === "decode") {
          ...
          const decoded = decodeBencode(inputData);
          const details = decodeBencodeWithDetails(inputData);
          ...
          if (state.inputFormat === "json") {
            const validation = validateJsonForBencode(state.input);
            if (!validation.isValid) {
              throw new Error(`Invalid JSON: ${validation.error}`);
            }
            inputData = validation.parsed;
          } else {
            const validation = validateYamlForBencode(state.input);
            if (!validation.isValid) {
          ...
          const encoded = encodeBencode(inputData);
        ...
        if (result.binary) {
          ...
          setBinaryMeta({ name: "output.bencode", size: result.binary.length });
          setOutput("");
        } else {
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <Label className="text-sm font-medium">
              {state.mode === "decode" ? "Bencode Data" : "Data to Encode"}
            </Label>
          ...
          <div className="relative">
            <Textarea
              ...
              placeholder={
                state.mode === "decode"
                  ? "Paste Base64/Hex encoded bencode data or upload a file..."
...
function renderFields(fields: BencodeField[], depth = 0): React.ReactNode[] {
