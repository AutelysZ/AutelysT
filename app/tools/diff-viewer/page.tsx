...
import {
  ...
  formatValue,
  type DiffChange,
} from "@/lib/data/json-diff";
import {
  computeTextDiff,
  getDiffStats,
  groupIntoHunks,
  type TextDiffLine,
} from "@/lib/data/text-diff";
...
function DiffViewerContent() {
  ...
  const [leftUploadError, setLeftUploadError] = React.useState<string | null>(
    null,
  );
  const [rightUploadError, setRightUploadError] = React.useState<string | null>(
    null,
  );
  ...
  const [leftFileContent, setLeftFileContent] = React.useState<string | null>(
    null,
  );
  const [rightFileContent, setRightFileContent] = React.useState<string | null>(
    null,
  );
  ...
  const handleLoadHistory = React.useCallback(
    (entry: HistoryEntry) => {
      ...
      if (params.leftFormat)
        setParam(
          "leftFormat",
          params.leftFormat as z.infer<typeof paramsSchema>["leftFormat"],
        );
      if (params.rightFormat)
        setParam(
          "rightFormat",
          params.rightFormat as z.infer<typeof paramsSchema>["rightFormat"],
        );
...
function DiffViewerInner({
  ...
  const leftDetected = React.useMemo(
    () => detectFormat(leftEffectiveText),
    [leftEffectiveText],
  );
  const rightDetected = React.useMemo(
    () => detectFormat(rightEffectiveText),
    [rightEffectiveText],
  );
  const leftTooLong = React.useMemo(
    () => oversizeKeys.includes("leftText"),
    [oversizeKeys],
  );
  const rightTooLong = React.useMemo(
    () => oversizeKeys.includes("rightText"),
    [oversizeKeys],
  );

  const leftResolved =
    state.leftFormat === "auto" ? leftDetected.format : state.leftFormat;
  const rightResolved =
    state.rightFormat === "auto" ? rightDetected.format : state.rightFormat;
  ...
  const { textDiff, textStats, diffSkipped } = React.useMemo(() => {
    const shouldAlignToLeftFormat =
      leftResolved !== "text" && rightResolved !== "text";
    ...
    if (!leftText && !rightText) {
      return {
        textDiff: [] as TextDiffLine[],
        textStats: { added: 0, removed: 0, unchanged: 0 },
        diffSkipped: false,
      };
    }
    ...
    const skip =
      leftBytes > FILE_SIZE_LIMIT ||
      rightBytes > FILE_SIZE_LIMIT ||
      leftLines > 200000 ||
      rightLines > 200000;
    if (skip) {
      return {
  ...
  const hasTextChanges = React.useMemo(
    () => textDiff.some((line) => line.type !== "unchanged"),
    [textDiff],
  );
  const changedLines = React.useMemo(
    () => textDiff.filter((line) => line.type !== "unchanged"),
    [textDiff],
  );
  const diffLineCount = changedLines.length;
  const diffByteSize = React.useMemo(
    () => getDiffByteSize(changedLines),
    [changedLines],
  );
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      upsertInputEntry(
        {
          leftText: leftFileName ? "" : state.leftText,
          rightText: rightFileName ? "" : state.rightText,
        },
        {
          leftFormat: state.leftFormat,
          rightFormat: state.rightFormat,
        ...
        leftFileName ||
          rightFileName ||
          state.leftText.slice(0, 50) +
            (state.rightText ? " vs " + state.rightText.slice(0, 50) : ""),
        {
  ...
  const content = (
    <div
      className={cn(
        "flex flex-col gap-4",
        isFullscreen &&
          "fixed inset-0 z-50 bg-background p-4 pt-0 overflow-y-auto",
      )}
    >
      {!isFullscreen && (
        <div className="flex flex-col gap-4 md:flex-row">
          <DiffInputPanel
            ...
            detectedFormat={
              state.leftFormat === "auto" ? leftDetected.format : null
            }
          ...
          <DiffInputPanel
            ...
            detectedFormat={
              state.rightFormat === "auto" ? rightDetected.format : null
            }
            showDetected={mounted}
      ...
      {showDiffContent ? (
        (showTabs && changes.length === 0) || (!showTabs && !hasTextChanges) ? (
          ...
          <div
            className={cn(
              "space-y-3",
              isFullscreen && "flex flex-1 flex-col relative pt-3",
            )}
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              {suppressDetail ? (
                ...
                <Tabs
                  ...
                  onValueChange={(v) =>
                    setParam("viewMode", v as ViewMode, true)
                  }
                  className="w-auto"
                >
            ...
            {diffSkipped ? (
              <div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
                Files are too large to diff in-browser. Reduce size or download
                files for external comparison.
              </div>
            ) : suppressDetail ? (
              <div className="rounded-md border border-dashed p-3 text-xs text-muted-foreground">
                Diff is too large to render ({diffLineCount.toLocaleString()}{" "}
                lines, {formatBytes(diffByteSize)}). Use the download button to
                view the full patch.
              </div>
              ...
              <div
                className={cn(
                  "rounded-md border font-mono text-sm",
                  isFullscreen ? "flex-1" : undefined,
                )}
              >
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {formatValue === "auto" && (
            <span className="text-xs text-muted-foreground">
              Detected: {showDetected ? detectedLabel : "—"}
            </span>
          )}
          <input
            ref={inputRef}
...
function buildUnifiedPatch(
  ...
  for (const hunk of hunks) {
    ...
    const rightCount = hunk.lines.filter(
      (line) => line.type !== "removed",
    ).length;
    output.push(
      `@@ -${hunk.startLeft},${leftCount} +${hunk.startRight},${rightCount} @@`,
    );
        ...
        {change.newValue !== undefined && (
          <div className="flex items-start gap-1">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                handleCopy(formatValue(change.newValue), `new-${change.path}`)
              }
              className="h-5 w-5 shrink-0 opacity-0 transition-opacity group-hover/cell:opacity-100"
            >
              {copied === `new-${change.path}` ? (
                <Check className="h-3 w-3" />
              ) : (
                <Copy className="h-3 w-3" />
              )}
    ...
    while (cursor < diffLines.length) {
      ...
      if (remaining > 0) {
        if (remaining < 3) {
          ...
          const { leftRange, rightRange } = getGapRanges(
            diffLines,
            gapStart,
            gapEnd,
          );
        ...
        return (
          ...
          >
                ...
                <button
                  ...
                  onClick={() =>
                    setExpandedGaps((prev) => ({
                      ...prev,
                      [item.id]: {
                        start: (prev[item.id]?.start ?? 0) + 10,
                        end: prev[item.id]?.end ?? 0,
                      },
                    }))
                  }
                >
                ...
                <button
                  ...
                  onClick={() =>
                    setExpandedGaps((prev) => ({
                      ...prev,
                      [item.id]: {
                        start: prev[item.id]?.start ?? 0,
                        end: (prev[item.id]?.end ?? 0) + 10,
                      },
                    }))
                  }
                >
            ...
            <div className="flex-1 font-mono text-sm text-muted-foreground">
              @@ -{item.leftRange.start},{item.leftRange.count} +
              {item.rightRange.start},{item.rightRange.count} @@
            </div>
          </div>
        );
      })}
...
function TextDiffLineRow({ line }: { line: TextDiffLine }) {
  ...
  const prefix =
    line.type === "added" ? "+" : line.type === "removed" ? "-" : " ";
  const hasCharDiff = Boolean(
    line.charDiff && (line.type === "added" || line.type === "removed"),
  );
...
function getGapRanges(lines: TextDiffLine[], start: number, end: number) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    if (leftStart === undefined && line.lineNumber?.left !== undefined)
      leftStart = line.lineNumber.left;
    if (rightStart === undefined && line.lineNumber?.right !== undefined)
      rightStart = line.lineNumber.right;
  }
  ...
  return {
    leftRange: { start: leftStart, count: leftCount },
    rightRange: { start: rightStart, count: rightCount },
  };
}

function findPrevLineNumber(
  lines: TextDiffLine[],
  start: number,
  side: "left" | "right",
) {
  for (let i = start - 1; i >= 0; i--) {
    const value =
      side === "left" ? lines[i].lineNumber?.left : lines[i].lineNumber?.right;
